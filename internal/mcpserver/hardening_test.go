...
func TestAuditLogger_SensitiveRedaction(t *testing.T) {
	...
	sanitized := SanitizeMetadata(sensitive)
...
func TestRateLimiter_Allows(t *testing.T) {
	limiter := NewRateLimiter(10, 1.0) // 10 burst, 1 token/sec
...
func TestRateLimiter_Refills(t *testing.T) {
	limiter := NewRateLimiter(2, 10.0) // 2 burst, fast refill
...
func TestRateLimiter_Concurrent(t *testing.T) {
	limiter := NewRateLimiter(100, 1000.0)
...
func TestQuotaTracker_RequestLimit(t *testing.T) {
	tracker := NewQuotaTracker(QuotaLimits{MaxRequestsPerContext: 3})
...
func TestQuotaTracker_NavigationLimit(t *testing.T) {
	tracker := NewQuotaTracker(QuotaLimits{MaxNavigationsPerContext: 2})
...
func TestQuotaTracker_MemoryLimit(t *testing.T) {
	tracker := NewQuotaTracker(QuotaLimits{MaxMemoryPerContext: 100})
...
func TestQuotaTracker_Release(t *testing.T) {
	tracker := NewQuotaTracker(QuotaLimits{MaxRequestsPerContext: 5})
...
func TestHealthReporter_BasicMetrics(t *testing.T) {
	reporter := NewHealthReporter(10, func() int64 { return 3 })
...
func TestHealthReporter_Health(t *testing.T) {
	reporter := NewHealthReporter(100, func() int64 { return 5 })
...
func TestHealthReporter_UnhealthyOnMaxContexts(t *testing.T) {
	reporter := NewHealthReporter(5, func() int64 { return 10 })
...
func TestShutdownHandler_Trigger(t *testing.T) {
	handler := NewShutdownHandler(100*time.Millisecond, nil)

	assert.Equal(t, ShutdownNone, handler.Signal())

	handler.Trigger(ShutdownGraceful)
	assert.Equal(t, ShutdownGraceful, handler.Signal())

	// Duplicate trigger should not panic
	handler.Trigger(ShutdownStdin)
	assert.Equal(t, ShutdownGraceful, handler.Signal()) // first signal wins
}

// TestShutdownHandler_Wait tests wait behavior.
func TestShutdownHandler_Wait(t *testing.T) {
	handler := NewShutdownHandler(50*time.Millisecond, nil)
	...
	select {
	case sig := <-done:
		assert.Equal(t, ShutdownGraceful, sig)
...
func TestShutdownHandler_Execute(t *testing.T) {
	called := false
	handler := NewShutdownHandler(100*time.Millisecond, func(ctx context.Context) error {
		called = true
		return nil
	})
