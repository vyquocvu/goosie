...
func (m *mockTransport) startServer(t *testing.T, server *Server) {
	go func() {
		defer close(m.done)
...
func (m *mockTransport) handleMessage(t *testing.T, msg *MCPMessage, server *Server) *MCPMessage {
...
func TestToolSchema_Navigate_RequiredFields(t *testing.T) {
	schema, ok := GetToolSchema("browser_navigate")
	assert.True(t, ok)
