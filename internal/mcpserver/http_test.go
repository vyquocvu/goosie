...
func startTestHTTPServer(t *testing.T, config HTTPConfig) (*HTTPServer, string) {
	t.Helper()
	bc := browsercontrol.NewEngineService()
	srv, err := NewServer(bc, ServerOptions{
		Name:        "test",
		Version:     "1.0.0",
		MaxContexts: 10,
	})
	...
	hs, err := NewHTTPServer(srv, config)
	require.NoError(t, err)
...
func TestHTTPServer_LoopbackOnly(t *testing.T) {
	bc := browsercontrol.NewEngineService()
	srv, _ := NewServer(bc, ServerOptions{})

	// Non-loopback should be rejected
	_, err := NewHTTPServer(srv, HTTPConfig{Bind: "0.0.0.0"})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "loopback")

	_, err = NewHTTPServer(srv, HTTPConfig{Bind: "192.168.1.1"})
	assert.Error(t, err)

	// Loopback should be accepted
	_, err = NewHTTPServer(srv, HTTPConfig{Bind: "127.0.0.1"})
	assert.NoError(t, err)

	_, err = NewHTTPServer(srv, HTTPConfig{Bind: "localhost"})
	assert.NoError(t, err)
}
...
func TestHTTPServer_InitializeSession(t *testing.T) {
	...
	resp, err := http.Post(baseURL+DefaultHTTPConfig().Path, "application/json", bytes.NewReader(body))
	require.NoError(t, err)
	defer resp.Body.Close()
...
func TestHTTPServer_ToolsList(t *testing.T) {
	hs, baseURL := startTestHTTPServer(t, DefaultHTTPConfig())
	_ = hs
	...
	httpReq, _ := http.NewRequest("POST", baseURL+DefaultHTTPConfig().Path, bytes.NewReader(body))
...
func TestHTTPServer_OriginValidation(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", baseURL+DefaultHTTPConfig().Path, body)
...
func TestHTTPServer_HostValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.host, func(t *testing.T) {
			...
			hs, _ := NewHTTPServer(nil, DefaultHTTPConfig())
...
func TestHTTPServer_DeleteSession(t *testing.T) {
	...
	req, _ := http.NewRequest("DELETE", baseURL+DefaultHTTPConfig().Path, nil)
...
func TestHTTPServer_InvalidJSON(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", baseURL+DefaultHTTPConfig().Path, bytes.NewReader([]byte(`not json`)))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Mcp-Session-Id", sessionID)
...
func TestRouteHTTPRequest_UnknownMethod(t *testing.T) {
	bc := browsercontrol.NewEngineService()
	srv, _ := NewServer(bc, ServerOptions{})
	hs, err := NewHTTPServer(srv, DefaultHTTPConfig())
	require.NoError(t, err)
...
func TestRouteHTTPRequest_Ping(t *testing.T) {
	bc := browsercontrol.NewEngineService()
	srv, _ := NewServer(bc, ServerOptions{})
	hs, _ := NewHTTPServer(srv, DefaultHTTPConfig())
...
func createSession(t *testing.T, baseURL string) string {
	t.Helper()
	body := bytes.NewReader([]byte(`{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"` + ProtocolVersion + `"}}`))
	resp, err := http.Post(baseURL+DefaultHTTPConfig().Path, "application/json", body)
	require.NoError(t, err)
	defer resp.Body.Close()
