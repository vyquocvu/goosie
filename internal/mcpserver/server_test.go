...
func TestServer_NewServer(t *testing.T) {
	svc := newFakeService()
	server, err := NewServer(svc, ServerOptions{
		Name:    "test",
		Version: "0.0.1",
	})
...
func TestToolSchemas_GetSchema_NotFound(t *testing.T) {
	_, ok := GetToolSchema("nonexistent_tool")
	assert.False(t, ok)
}
...
func TestContains(t *testing.T) {
	ss := []string{"a", "b", "c"}
	assert.True(t, contains(ss, "a"))
	assert.False(t, contains(ss, "d"))
}
...
func TestErrorResponse_JSON(t *testing.T) {
	resp := newErrorResponse(browsercontrol.ErrPageChanged, "reference outdated", true)
