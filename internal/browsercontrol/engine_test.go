...
func newTestContext(t *testing.T, s *EngineService) (context.Context, Context, func()) {
	t.Helper()
...
func TestNavigate_StateSequence(t *testing.T) {
	...
	nav, err := c.Navigate(ctx, srv.URL, WaitComplete, 5000)
	require.NoError(t, err)
	...
	info, err := c.Info(ctx)
	require.NoError(t, err)
	assert.Equal(t, ContextComplete, info.State)
}
...
func TestNavigate_WaitCommit(t *testing.T) {
	...
	nav, err := c.Navigate(ctx, srv.URL, WaitCommit, 5000)
	require.NoError(t, err)
	assert.True(t, nav.WaitConditionMet)
	assert.Equal(t, 1, nav.PageRevision)
}
...
func TestNavigate_WaitInteractive(t *testing.T) {
	...
	nav, err := c.Navigate(ctx, srv.URL, WaitInteractive, 5000)
	require.NoError(t, err)
	assert.True(t, nav.WaitConditionMet)
	assert.Equal(t, 1, nav.PageRevision)
}
...
func TestNavigate_RevisionIncrements(t *testing.T) {
	...
	nav1, err := c.Navigate(ctx, srv.URL, WaitComplete, 5000)
	require.NoError(t, err)
	assert.Equal(t, 1, nav1.PageRevision)

	nav2, err := c.Navigate(ctx, srv.URL, WaitComplete, 5000)
	require.NoError(t, err)
...
func TestNavigate_InvalidMIME(t *testing.T) {
	...
	_, err := c.Navigate(ctx, srv.URL, WaitComplete, 5000)
	require.Error(t, err)
	var be *Error
	assert.ErrorAs(t, err, &be)
	assert.Equal(t, ErrUnsupported, be.Code)
}
...
func TestNavigate_Cancellation(t *testing.T) {
	...
	_, err := c.Navigate(navCtx, srv.URL, WaitComplete, 5000)
	require.Error(t, err)
...
func TestNavigate_Timeout(t *testing.T) {
	...
	_, err := c.Navigate(ctx, srv.URL, WaitComplete, 100)
	require.Error(t, err)
}
...
func TestCloseDuringNavigation(t *testing.T) {
	...
	go func() {
		ec.Navigate(ctx, srv.URL, WaitComplete, 5000)
	}()
...
func TestViewport_SetAndGet(t *testing.T) {
	...
	vp, err := c.SetViewport(ctx, Viewport{Width: 800, Height: 600, Scale: 2.0})
	require.NoError(t, err)
...
func TestNavigate_FileURLDenied(t *testing.T) {
	...
	_, err := c.Navigate(ctx, "http://0.0.0.0:1/test", WaitComplete, 500)
	require.Error(t, err)
}
...
func TestNavigate_Redirect(t *testing.T) {
	...
	nav, err := c.Navigate(ctx, srv.URL+"/redirect", WaitComplete, 5000)
	require.NoError(t, err)
...
func TestNavigate_OversizedResponse(t *testing.T) {
	...
	_, err := c.Navigate(ctx, srv.URL, WaitComplete, 2000)
...
func TestClick_RevisionMismatch(t *testing.T) {
	...
	_, err := c.Click(ctx, ref, ClickOptions{Button: "left", TimeoutMs: 5000})
	require.Error(t, err)
	var be *Error
	assert.ErrorAs(t, err, &be)
	assert.Equal(t, ErrPageChanged, be.Code)
}
...
func TestType_RevisionMismatch(t *testing.T) {
	...
	ref := ElementRef{Ref: "e_1_abc", ContextID: c.ID(), PageRevision: 99}
	_, err := c.Type(ctx, ref, "hello", TypeOptions{Replace: true})
	require.Error(t, err)
	var be *Error
	assert.ErrorAs(t, err, &be)
	assert.Equal(t, ErrPageChanged, be.Code)
...
func TestClick_WrongContext(t *testing.T) {
	...
	ref := ElementRef{Ref: "e_1_abc", ContextID: "other-ctx", PageRevision: 1}
	_, err := c.Click(ctx, ref, ClickOptions{})
	require.Error(t, err)
	var be *Error
	assert.ErrorAs(t, err, &be)
...
func TestSnapshot_Truncation(t *testing.T) {
	...
	snap, err := c.Snapshot(ctx, SnapshotOptions{MaxNodes: 5, MaxDepth: 10})
	require.NoError(t, err)
...
func collectRoles(nodes []SemanticNode) []string {
	var roles []string
	for _, n := range nodes {
