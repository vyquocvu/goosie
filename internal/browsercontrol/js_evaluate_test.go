...
func TestJS_Evaluate_Basic(t *testing.T) {
	...
	// Test string return
	result, err := ec.Evaluate(ctx, `"hello world"`, EvaluateOptions{})
	require.NoError(t, err)
...
func TestJS_Evaluate_SourceLengthLimit(t *testing.T) {
	...
	longSource := make([]byte, MaxSourceBytes+100)
	for i := range longSource {
		longSource[i] = 'x'
	}

	result, err := ec.Evaluate(ctx, string(longSource), EvaluateOptions{})
	require.NoError(t, err)
	assert.True(t, result.IsError)
...
func TestJS_Evaluate_Cancellation(t *testing.T) {
	...
	_, err = ec.Evaluate(cancelledCtx, `"test"`, EvaluateOptions{})
	require.Error(t, err)
...
func TestJS_Evaluate_Function(t *testing.T) {
	...
	result, err := ec.Evaluate(ctx, `(function(x, y) { return x + y; })(5, 3)`, EvaluateOptions{})
	require.NoError(t, err)
	assert.Equal(t, "number", result.Type)
	if intVal, ok := result.Value.(int64); ok {
