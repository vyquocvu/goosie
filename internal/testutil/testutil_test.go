...
func TestRenderToImage(t *testing.T) {
	...
	img, err := RenderToImage(rect, 100, 100)
	if err != nil {
...
func TestRenderToImage_NilObject(t *testing.T) {
	_, err := RenderToImage(nil, 100, 100)
	if err == nil {
...
func TestSaveImageAsPNG(t *testing.T) {
	...
	err := SaveImageAsPNG(img, filePath)
	if err != nil {
...
func TestSaveImageAsPNG_CreatesDirectory(t *testing.T) {
	...
	err := SaveImageAsPNG(img, nestedPath)
	if err != nil {
...
func TestShouldSaveScreenshots(t *testing.T) {
	// Save original value
	original := os.Getenv(ScreenshotDir)
	defer os.Setenv(ScreenshotDir, original)

	// Test when not set
	os.Unsetenv(ScreenshotDir)
	if ShouldSaveScreenshots() {
		t.Error("ShouldSaveScreenshots should return false when env is not set")
	}

	// Test when set
	os.Setenv(ScreenshotDir, "/tmp/screenshots")
	if !ShouldSaveScreenshots() {
...
func TestSaveTestScreenshot_Disabled(t *testing.T) {
	...
	path, err := SaveTestScreenshot(rect, "test", 100, 100)
...
func TestSaveTestScreenshot_Enabled(t *testing.T) {
	...
	// Enable screenshots
	os.Setenv(ScreenshotDir, tempDir)

	rect := canvas.NewRectangle(color.RGBA{R: 0, G: 0, B: 255, A: 255})
	path, err := SaveTestScreenshot(rect, "TestExample", 100, 100)
