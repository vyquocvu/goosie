...
func TestCSPPolicy_HasDirective(t *testing.T) {
	p := ParseCSPHeader("script-src 'self'; style-src 'self'")
...
func TestCSPPolicy_AllowsURL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := &CSPPolicy{directives: make(map[string][]string)}
			if len(tt.sources) > 0 {
				p.directives[tt.directive] = tt.sources
			}
			if tt.rawURL == "" {
...
func TestCSPPolicy_AllowsURL_InvalidURL(t *testing.T) {
	p := ParseCSPHeader("script-src 'self'")
	base := mustParseURL("https://example.com")
...
func TestAllowScript(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			policy:  ParseCSPHeader("script-src 'self'"),
			rawURL:  "https://example.com/app.js",
			wantErr: nil,
		},
		{
			...
			wantErr: ErrCSPViolation,
		},
		{
			name:    "explicit host allowed",
			policy:  ParseCSPHeader("script-src *.example.com"),
		...
		},
		{
			...
			policy:  ParseCSPHeader("script-src 'none'"),
			rawURL:  "https://example.com/app.js",
			wantErr: ErrCSPViolation,
		},
		{
		...
		},
		{
			...
			policy:  ParseCSPHeader("script-src https:"),
...
func TestAllowStyle(t *testing.T) {
	...
	p := ParseCSPHeader("style-src 'self' *.fonts.example.com")
...
func TestAllowConnect(t *testing.T) {
	...
	p := ParseCSPHeader("connect-src api.example.com")
...
func TestAllowBaseURI(t *testing.T) {
	...
	p := ParseCSPHeader("base-uri 'self'")
...
func TestAllowScript_InlineScript(t *testing.T) {
	...
	p2 := ParseCSPHeader("default-src 'self'")
	...
	p3 := ParseCSPHeader("img-src 'self'")
...
func TestMatchesHostWildcard(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.host+"/"+tt.domain, func(t *testing.T) {
			got := matchesHostWildcard(tt.host, tt.domain)
			if got != tt.want {
				t.Errorf("matchesHostWildcard(%q, %q) = %v, want %v", tt.host, tt.domain, got, tt.want)
			}
		})
	}
}
...
func TestAllowsSource(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := AllowsSource(tt.sources, target, base)
			if got != tt.want {
...
func TestCSPPolicy_DirectiveFallback(t *testing.T) {
	...
	p := ParseCSPHeader("default-src 'self'")
	base := mustParseURL("https://example.com")
	...
	p2 := ParseCSPHeader("default-src 'none'; script-src *.example.com")
...
func BenchmarkParseCSPHeader(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		_ = ParseCSPHeader(header)
	}
}

func BenchmarkCSPPolicy_AllowsURL(b *testing.B) {
	p := ParseCSPHeader("default-src 'self'; script-src 'self' https://cdn.example.com; connect-src https://api.example.com")
	base := mustParseURL("https://example.com/page")
...
func BenchmarkAllowScript(b *testing.B) {
	p := ParseCSPHeader("script-src 'self' https://cdn.example.com")
	base := mustParseURL("https://example.com/page")
	rawURL := "https://cdn.example.com/app.js"
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
