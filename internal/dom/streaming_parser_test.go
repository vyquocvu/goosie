...
func storeTree(t *testing.T, doc *Document) string {
	t.Helper()
	...
	walk = func(id NodeID, depth int) {
		...
		for child := doc.Store.FirstChild(id); child != NodeNone; child = doc.Store.NextSibling(child) {
			walk(child, depth+1)
		}
	}
	walk(doc.Root, 0)
...
func findElement(t *testing.T, doc *Document, tagName atom.Atom) NodeID {
	t.Helper()
	for id := NodeID(1); id <= NodeID(doc.Store.NodeCount()); id++ {
		if doc.Store.Kind(id) == NodeKindElement && doc.Store.Name(id) == tagName {
			return id
		}
	}
	return NodeNone
...
func findAllElements(t *testing.T, doc *Document, tagName atom.Atom) []NodeID {
	t.Helper()
	var result []NodeID
	for id := NodeID(1); id <= NodeID(doc.Store.NodeCount()); id++ {
		if doc.Store.Kind(id) == NodeKindElement && doc.Store.Name(id) == tagName {
...
func collectTexts(t *testing.T, store *Store, id NodeID) []string {
	t.Helper()
	var texts []string
...
func compareStructure(t *testing.T, oldRoot *html.Node, doc *Document) {
	t.Helper()
	...
	var walkNew func(NodeID, int)
	walkNew = func(id NodeID, depth int) {
		if id == NodeNone {
			return
		}
		...
		for c := doc.Store.FirstChild(id); c != NodeNone; c = doc.Store.NextSibling(c) {
			walkNew(c, depth+1)
		}
	}
	walkNew(doc.Root, 0)
...
func TestStreamParseSimpleHTML(t *testing.T) {
	...
	t.Run("new streaming API", func(t *testing.T) {
		doc, err := parser.ParseDocumentCtx(context.Background(), strings.NewReader(input), ParseConfig{})
		require.NoError(t, err)
		...
		assert.NotEqual(t, NodeNone, doc.Root, "Root must be a valid NodeID")
		...
		assert.NotEqual(t, NodeNone, htmlID, "document should have html child")

		// Walk structure: document → html → head + body.
		headID := findElement(t, doc, atom.AtomHead)
		bodyID := findElement(t, doc, atom.AtomBody)
		assert.NotEqual(t, NodeNone, headID, "should have head element")
		assert.NotEqual(t, NodeNone, bodyID, "should have body element")

		// Verify text "Hello" under body → p.
		pID := findElement(t, doc, atom.AtomP)
		assert.NotEqual(t, NodeNone, pID, "should have p element")
	...
	t.Run("old and new produce consistent results", func(t *testing.T) {
		...
		newDoc, err := parser.ParseDocumentCtx(context.Background(), strings.NewReader(input), ParseConfig{})
		require.NoError(t, err)
...
func TestStreamParseTextNodes(t *testing.T) {
	...
	t.Run("consistency", func(t *testing.T) {
		...
		newDoc, _ := parser.ParseDocumentCtx(context.Background(), strings.NewReader(input), ParseConfig{})
		compareStructure(t, oldDoc, newDoc)
	})
}
...
func TestStreamParseAttributes(t *testing.T) {
	...
	t.Run("new streaming API attributes", func(t *testing.T) {
		...
		require.NotEqual(t, NodeNone, divID)
		...
		require.NotEqual(t, NodeNone, aID)
...
func TestStreamParseVoidElements(t *testing.T) {
	...
	t.Run("new streaming API void elements", func(t *testing.T) {
		...
		for _, id := range brs {
			assert.Equal(t, NodeNone, doc.Store.FirstChild(id), "br should have no children")
		}
		for _, id := range imgs {
			assert.Equal(t, NodeNone, doc.Store.FirstChild(id), "img should have no children")
		...
		for _, id := range inputs {
			assert.Equal(t, NodeNone, doc.Store.FirstChild(id), "input should have no children")
		}
	})
...
func TestStreamParseNestedElements(t *testing.T) {
	...
	t.Run("new streaming API nesting", func(t *testing.T) {
		...
		require.NotEqual(t, NodeNone, spanID)

		// Walk up parents to verify depth.
		depth := 0
		for p := doc.Store.Parent(spanID); p != NodeNone; p = doc.Store.Parent(p) {
			depth++
		}
		...
		for i := 0; i < 3; i++ {
			assert.NotEqual(t, NodeNone, parent, "should have at least 3 div ancestors")
...
func TestStreamParseMalformedHTML(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			newDoc, err := parser.ParseDocumentCtx(context.Background(), strings.NewReader(tc.input), ParseConfig{})
			require.NoError(t, err, "new API should not error on malformed HTML")
			require.NotNil(t, newDoc)

			// Both produce valid document roots.
			assert.Equal(t, NodeKindDocument, newDoc.Store.Kind(newDoc.Root))
...
func TestStreamParseEmptyReader(t *testing.T) {
	...
	t.Run("new streaming API empty", func(t *testing.T) {
		doc, err := parser.ParseDocumentCtx(context.Background(), strings.NewReader(""), ParseConfig{})
		require.NoError(t, err)
		require.NotNil(t, doc)
		assert.Equal(t, NodeKindDocument, doc.Store.Kind(doc.Root),
	...
	t.Run("consistency", func(t *testing.T) {
		...
		newDoc, _ := parser.ParseDocumentCtx(context.Background(), strings.NewReader(""), ParseConfig{})
		compareStructure(t, oldDoc, newDoc)
	})
}
...
func TestStreamParseCancellation(t *testing.T) {
	...
	t.Run("slow reader with cancellation", func(t *testing.T) {
		...
		doc, err := parser.ParseDocumentCtx(ctx, slowReader, ParseConfig{})
...
func TestStreamParseMaxBuf(t *testing.T) {
	...
	t.Run("zero MaxBuf means unlimited", func(t *testing.T) {
		...
		cfg := ParseConfig{MaxBuf: 0} // zero → no limit
...
func TestStreamParseStoreAppendAttrs(t *testing.T) {
	...
	div, err := s.Allocate()
	require.NoError(t, err)
	require.NoError(t, s.SetKind(div, NodeKindElement))
	require.NoError(t, s.SetName(div, atom.AtomDiv))

	span, err := s.Allocate()
	require.NoError(t, err)
	require.NoError(t, s.SetKind(span, NodeKindElement))
...
func TestStreamParseStoreAppendText(t *testing.T) {
	...
	t1, err := s.Allocate()
	require.NoError(t, err)
	require.NoError(t, s.SetKind(t1, NodeKindText))

	t2, err := s.Allocate()
	require.NoError(t, err)
	require.NoError(t, s.SetKind(t2, NodeKindText))
...
func TestStreamParseNilContext(t *testing.T) {
	...
	doc, err := parser.ParseDocumentCtx(nil, strings.NewReader(input), ParseConfig{})
	...
	pID := findElement(t, doc, atom.AtomP)
	assert.NotEqual(t, NodeNone, pID, "should find p element")
}
...
func BenchmarkStreamParseDocumentCtx(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		_, err := parser.ParseDocumentCtx(context.Background(), strings.NewReader(input), ParseConfig{})
		if err != nil {
			b.Fatal(err)
		}
	}
