...
func TestAllocateAndBasicOperations(t *testing.T) {
	...
	// Set kind.
	if err := s.SetKind(doc, NodeKindDocument); err != nil {
		t.Fatal(err)
	}
	if s.Kind(doc) != NodeKindDocument {
	...
	if err := s.SetKind(div, NodeKindElement); err != nil {
		t.Fatal(err)
	}
...
func TestParentChildRelationships(t *testing.T) {
	...
	div1, _ := s.Allocate()
	s.SetKind(div1, NodeKindElement)
	s.SetName(div1, atom.AtomDiv)

	div2, _ := s.Allocate()
	s.SetKind(div2, NodeKindElement)
	s.SetName(div2, atom.AtomDiv)

	p, _ := s.Allocate()
	s.SetKind(p, NodeKindElement)
...
func TestInsertBefore(t *testing.T) {
	...
	div3, _ := s.Allocate()
	s.SetKind(div3, NodeKindElement)
...
func TestStoreRemoveChild(t *testing.T) {
	...
	if s.PrevSibling(div3) != NodeNone {
	...
	if s.FirstChild(doc) != NodeNone {
		t.Errorf("FirstChild = %v, want NodeNone", s.FirstChild(doc))
	}
	if s.LastChild(doc) != NodeNone {
...
func TestRemoveSubtree(t *testing.T) {
	...
	p1, _ := s.Allocate()
	s.SetKind(p1, NodeKindElement)
	s.SetName(p1, atom.AtomP)

	p2, _ := s.Allocate()
	s.SetKind(p2, NodeKindElement)
...
func TestReplace(t *testing.T) {
	...
	old, _ := s.Allocate()
	s.SetKind(old, NodeKindElement)
	s.SetName(old, atom.AtomDiv)

	new, _ := s.Allocate()
	s.SetKind(new, NodeKindElement)
...
func TestReset(t *testing.T) {
	...
	s.AppendChild(doc, div)
	s.SetText(div, "test")
	s.SetAttrs(div, []Attr{{Name: atom.AttrId, Value: atom.Intern("test")}})
...
func TestFlags(t *testing.T) {
	...
	if s.HasFlag(div, NodeFlagDirty) {
		t.Error("should not have Dirty flag initially")
	}

	if err := s.SetFlag(div, NodeFlagDirty); err != nil {
		t.Fatal(err)
	}
	if !s.HasFlag(div, NodeFlagDirty) {
	...
	if err := s.ClearFlag(div, NodeFlagDirty); err != nil {
		t.Fatal(err)
	}
	if s.HasFlag(div, NodeFlagDirty) {
...
func TestRareData(t *testing.T) {
	...
	data := rareData{Namespace: atom.Intern("svg")}
...
func TestEdgeCases(t *testing.T) {
	...
	if _, err := s.Attrs(999); err != ErrInvalidNodeID {
		t.Errorf("Attrs(999) error = %v, want ErrInvalidNodeID", err)
	}
	if _, err := s.Text(999); err != ErrInvalidNodeID {
	...
	div, _ := s.Allocate()
	s.SetKind(div, NodeKindElement)
	if err := s.AppendChild(NodeNone, div); err != ErrInvalidParent {
	...
	doc, _ := s.Allocate()
	s.SetKind(doc, NodeKindDocument)
	if err := s.AppendChild(doc, NodeNone); err != ErrInvalidNodeID {
	...
	div2, _ := s.Allocate()
	s.SetKind(div2, NodeKindElement)
	if err := s.RemoveChild(doc, div2); err != ErrNodeNotFound {
...
func TestTextOffsetUpdates(t *testing.T) {
	...
	text1, _ := s.Allocate()
	s.SetKind(text1, NodeKindText)
	s.SetText(text1, "First")

	text2, _ := s.Allocate()
	s.SetKind(text2, NodeKindText)
...
func TestMoveChild(t *testing.T) {
	...
	doc1, _ := s.Allocate()
	s.SetKind(doc1, NodeKindDocument)

	doc2, _ := s.Allocate()
	s.SetKind(doc2, NodeKindDocument)
