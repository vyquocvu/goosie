...
func TestM54_StoreProvidesFullTreeAccess(t *testing.T) {
	...
	doc, _ := s.Allocate()
	_ = s.SetKind(doc, NodeKindDocument)

	htmlID, _ := s.Allocate()
	_ = s.SetKind(htmlID, NodeKindElement)
	_ = s.SetName(htmlID, atom.AtomHtml)
	_ = s.AppendChild(doc, htmlID)

	body, _ := s.Allocate()
	_ = s.SetKind(body, NodeKindElement)
	_ = s.SetName(body, atom.AtomBody)
	_ = s.AppendChild(htmlID, body)

	div, _ := s.Allocate()
	_ = s.SetKind(div, NodeKindElement)
	_ = s.SetName(div, atom.AtomDiv)
	classVal := atom.Intern("main")
	_ = s.AppendAttrs(div, []Attr{{Name: atom.AttrClass, Value: classVal}})
	_ = s.AppendChild(body, div)

	p, _ := s.Allocate()
	_ = s.SetKind(p, NodeKindElement)
	_ = s.SetName(p, atom.AtomP)
	_ = s.AppendChild(div, p)

	text, _ := s.Allocate()
	_ = s.SetKind(text, NodeKindText)
...
func TestM54_StoreSupportsMutation(t *testing.T) {
	...
	var order []atom.Atom
	for c := s.FirstChild(root); c != NodeNone; c = s.NextSibling(c) {
	...
	_ = s.SetKind(replacement, NodeKindElement)
...
func TestM54_ParserProducesValidStore(t *testing.T) {
	...
	p := NewParser()
	doc, err := p.ParseDocumentCtx(context.Background(), strings.NewReader(input), ParseConfig{})
	if err != nil {
		t.Fatal(err)
	}
	...
	for it := doc.Store.Subtree(doc.Root); it.Next(); {
		id := it.ID()
		if doc.Store.Kind(id) != NodeKindElement {
			continue
		}
