...
func TestRecorderDebugLogDisabledByDefault(t *testing.T) {
	buf := &syncBuffer{}
	r := NewRecorder(1, "https://example.com")
	r.Finalize()
...
func TestRecorderDebugLogEmitsStructured(t *testing.T) {
	...
	r := NewRecorder(7, "https://example.com/page")
	r.SetDebugLog(logger)
	r.BeginPhase(PhaseParse)
	r.EndPhase(PhaseParse)
	r.AddCounters(Counters{NodeCount: 120, RuleCount: 30, ImageCount: 4})
...
func TestRecorderDebugLogNotEmittedWhenDisabledAfterSet(t *testing.T) {
	...
	r.AddCounters(Counters{NodeCount: 5})
...
func TestRecorderLogStructuredExplicitCall(t *testing.T) {
	...
	r := NewRecorder(3, "https://example.com/x")
	r.SetDebugLog(logger)
	r.BeginPhase(PhaseLayout)
	r.EndPhase(PhaseLayout)
	r.AddCounters(Counters{BoxCount: 42})
...
func TestRecorderDebugLogContextPropagation(t *testing.T) {
	...
	r := NewRecorder(9, "https://example.com")
	r.SetDebugLog(logger)
	r.LogStructured(ctx)
	r.Finalize()
