...
func sampleMetrics() Metrics {
	...
	return Metrics{
		...
		EndedAt:   end,
		Timings: []Timing{
			{Phase: PhaseNavigation, Started: start, Ended: start.Add(1 * time.Millisecond)},
			{Phase: PhaseDNSResolve, Started: start.Add(1 * time.Millisecond), Ended: start.Add(3 * time.Millisecond)},
			{Phase: PhaseConnect, Started: start.Add(3 * time.Millisecond), Ended: start.Add(7 * time.Millisecond)},
			{Phase: PhaseFirstByte, Started: start.Add(7 * time.Millisecond), Ended: start.Add(15 * time.Millisecond)},
			{Phase: PhaseBodyRead, Started: start.Add(15 * time.Millisecond), Ended: start.Add(35 * time.Millisecond)},
			{Phase: PhaseParse, Started: start.Add(35 * time.Millisecond), Ended: start.Add(95 * time.Millisecond)},
			{Phase: PhaseStyle, Started: start.Add(95 * time.Millisecond), Ended: start.Add(200 * time.Millisecond)},
			{Phase: PhaseLayout, Started: start.Add(200 * time.Millisecond), Ended: start.Add(330 * time.Millisecond)},
			{Phase: PhasePaint, Started: start.Add(330 * time.Millisecond), Ended: start.Add(420 * time.Millisecond)},
			{Phase: PhaseRaster, Started: start.Add(420 * time.Millisecond), Ended: start.Add(490 * time.Millisecond)},
			{Phase: PhasePresent, Started: start.Add(490 * time.Millisecond), Ended: end},
		},
		Counters: Counters{
			NodeCount:         432,
			RuleCount:         17,
...
func TestNewTimingPanel_HappyPath(t *testing.T) {
	p := NewTimingPanel(sampleMetrics())
	if p.NavID != 7 {
...
func TestNewTimingPanel_StatusClassification(t *testing.T) {
	p := NewTimingPanel(sampleMetrics())
	want := map[Phase]StatusKind{
		PhaseNavigation: StatusOK,
		PhaseDNSResolve: StatusOK,
		PhaseConnect:    StatusOK,
		PhaseFirstByte:  StatusOK,
		PhaseBodyRead:   StatusOK,
		PhaseParse:      StatusWarning, // ~60ms (>=50ms, <250ms)
		PhaseStyle:      StatusWarning, // ~105ms
		PhaseLayout:     StatusWarning, // ~130ms (>=50ms, <250ms)
		PhasePaint:      StatusWarning, // ~90ms
		PhaseRaster:     StatusWarning, // ~70ms
		PhasePresent:    StatusOK,      // ~10ms
	}
	for _, r := range p.Rows {
...
func TestNewTimingPanel_OverallStatus(t *testing.T) {
	p := NewTimingPanel(sampleMetrics())
	if p.OverallStatus != StatusWarning {
...
func TestNewTimingPanel_EmptyMetrics(t *testing.T) {
	m := Metrics{NavID: 1, URL: "https://empty.test"}
	p := NewTimingPanel(m)
...
func TestNewTimingPanel_SortByDuration(t *testing.T) {
	p := NewTimingPanelWith(sampleMetrics(), TimingPanelOptions{Sort: SortByDurationDesc})
	if len(p.Rows) < 2 {
...
func TestNewTimingPanel_SortByDurationAsc(t *testing.T) {
	p := NewTimingPanelWith(sampleMetrics(), TimingPanelOptions{Sort: SortByDurationAsc})
	if p.Rows[0].Duration > p.Rows[1].Duration {
...
func TestNewTimingPanel_IntervalCount(t *testing.T) {
	...
	m := Metrics{
		...
		Timings: []Timing{
			{Phase: PhaseLayout, Started: now, Ended: now.Add(20 * time.Millisecond)},
			{Phase: PhaseLayout, Started: now.Add(40 * time.Millisecond), Ended: now.Add(60 * time.Millisecond)},
		},
	}
	p := NewTimingPanel(m)
	for _, r := range p.Rows {
		if r.PhaseID == PhaseLayout {
...
func TestNewTimingPanel_CustomThresholds(t *testing.T) {
	...
	tight := &StatusThresholds{WarnAt: 5 * time.Millisecond, SlowAt: 10 * time.Millisecond}
	p := NewTimingPanelWith(m, TimingPanelOptions{StatusThresholds: tight})
	if p.Rows[0].Status != StatusSlow {
		t.Fatalf("under tight thresholds, layout Status = %s, want slow", p.Rows[0].Status)
	}

	// Wide thresholds: 20ms is "ok" under this policy.
	wide := &StatusThresholds{WarnAt: 100 * time.Millisecond, SlowAt: 500 * time.Millisecond}
	p = NewTimingPanelWith(m, TimingPanelOptions{StatusThresholds: wide})
	if p.Rows[0].Status != StatusOK {
...
func TestNewTimingPanel_DeterministicOutput(t *testing.T) {
	first := NewTimingPanel(sampleMetrics()).String()
	second := NewTimingPanel(sampleMetrics()).String()
	if first != second {
...
func TestClassifyBoundaryConditions(t *testing.T) {
	...
	cases := []struct {
		...
		{"zero", 0, StatusNone},
		{"one_ns", time.Nanosecond, StatusOK},
		{"below_warn", th.WarnAt - time.Millisecond, StatusOK},
		{"at_warn", th.WarnAt, StatusWarning},
		{"between", th.WarnAt + 10*time.Millisecond, StatusWarning},
		{"at_slow", th.SlowAt, StatusSlow},
		{"above_slow", th.SlowAt + time.Second, StatusSlow},
	}
	for _, c := range cases {
		c := c
