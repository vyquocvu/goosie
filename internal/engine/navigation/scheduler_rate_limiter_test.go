...
func TestSchedulerWithRateLimiter_AddResourceAcquires(t *testing.T) {
	sched := NewSchedulerWithOptions(SchedulerOptions{
		MaxConnsPerOrigin: 4,
		MaxConnsGlobal:    8,
	})
...
func TestSchedulerWithRateLimiter_CancelReleasesSlots(t *testing.T) {
	...
	r1, _ := sched.AddResource(context.Background(), "https://example.com/a.css", PriorityBlockingCSS)
	r2, _ := sched.AddResource(context.Background(), "https://example.com/b.css", PriorityScript)
	...
	r3, _ := sched.AddResource(ctx, "https://example.com/c.css", PriorityBlockingCSS)
	if !r3.ID.IsValid() {
		t.Fatal("AddResource after Cancel returned zero Load; slots were leaked")
	}

	r4, _ := sched.AddResource(ctx, "https://example.com/d.css", PriorityScript)
	if !r4.ID.IsValid() {
...
func TestSchedulerWithRateLimiter_CancelledContextReturnsZeroLoad(t *testing.T) {
	...
	load, retCtx := sched.AddResource(ctx, "https://example.com/b.css", PriorityScript)
	if load.ID.IsValid() {
...
func TestSchedulerWithRateLimiter_ConcurrentAddAndCancel(t *testing.T) {
	...
	for i := range rounds {
		...
		go func(i int) {
			...
			for range 4 {
				load, _ := sched.AddResource(ctx, "https://example.com/r.css", PriorityScript)
				if load.ID.IsValid() {
