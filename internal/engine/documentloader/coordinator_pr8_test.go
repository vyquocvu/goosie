...
func TestHandleDocumentEndDoesNotWaitForImages(t *testing.T) {
	...
	coord.HandleResource(Resource{Kind: KindImage, URL: "slow.png"})
	...
	if hasEvent(snap.Lifecycle, EventLoad) {
	...
	if !waitFor(t, func() bool { return hasEvent(h.cb.Snapshot().Lifecycle, EventLoad) }) {
...
func TestHandleDocumentEndDoesNotWaitForFonts(t *testing.T) {
	...
	coord.HandleResource(Resource{Kind: KindFont, URL: "f.woff2"})
...
func TestLateImageResultEmitsInDocumentOrder(t *testing.T) {
	...
	coord.HandleResource(Resource{Kind: KindImage, URL: "slow.png"})
	coord.HandleResource(Resource{Kind: KindCSS, URL: "fast.css"})
...
func hasEvent(events []LifecycleEvent, want LifecycleEvent) bool {
