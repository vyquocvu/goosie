...
func TestPaintChunkZeroValue(t *testing.T) {
	var chunk PaintChunk
	if chunk.Owner != LayoutNone {
...
func TestPaintChunkMarkClean(t *testing.T) {
	chunk := PaintChunk{Owner: LayoutID(1), Start: 0, End: 5}
	chunk.MarkDirty()
	if !chunk.Dirty() {
...
func TestPaintChunkCommandCount(t *testing.T) {
	chunk := PaintChunk{Start: 3, End: 10}
...
func TestPaintChunkIntersects(t *testing.T) {
	chunk := PaintChunk{
		Bounds: RectF{X: 0, Y: 0, W: 100, H: 100},
	}
	tests := []struct {
		name  string
		other RectF
		...
		{"overlap", RectF{X: 50, Y: 50, W: 100, H: 100}, true},
		{"inside", RectF{X: 10, Y: 10, W: 20, H: 20}, true},
		{"disjoint-right", RectF{X: 200, Y: 0, W: 50, H: 50}, false},
		{"disjoint-below", RectF{X: 0, Y: 200, W: 50, H: 50}, false},
		{"touching-edge", RectF{X: 100, Y: 0, W: 50, H: 50}, false}, // touching but not overlapping
	}
	for _, tt := range tests {
...
func TestPaintChunkListAddAndGet(t *testing.T) {
	cl := NewPaintChunkList()
	cl.Add(PaintChunk{Owner: LayoutID(1), Start: 0, End: 3})
	cl.Add(PaintChunk{Owner: LayoutID(2), Start: 3, End: 7})

	if cl.Len() != 2 {
		t.Fatalf("Len() = %d, want 2", cl.Len())
	}
	if got := cl.At(0).Owner; got != LayoutID(1) {
		t.Errorf("At(0).Owner = %d, want 1", got)
	}
	if got := cl.At(1).Owner; got != LayoutID(2) {
...
func TestSourceMappingAddAndLookup(t *testing.T) {
	sm := NewSourceMapping()
	sm.Add(LayoutID(1), 0, 5)
	sm.Add(LayoutID(2), 5, 10)

	start, end, ok := sm.Lookup(LayoutID(1))
	if !ok || start != 0 || end != 5 {
		t.Errorf("Lookup(1) = (%d, %d, %v), want (0, 5, true)", start, end, ok)
	}

	start, end, ok = sm.Lookup(LayoutID(2))
...
func TestSourceMappingEntries(t *testing.T) {
	sm := NewSourceMapping()
	sm.Add(LayoutID(1), 0, 5)
	sm.Add(LayoutID(3), 5, 10)

	entries := sm.Entries()
	if len(entries) != 2 {
		t.Fatalf("len(Entries()) = %d, want 2", len(entries))
	}
	if entries[0].Layout != LayoutID(1) || entries[0].CmdStart != 0 || entries[0].CmdEnd != 5 {
...
func TestSourceMappingOverwrite(t *testing.T) {
	sm := NewSourceMapping()
	sm.Add(LayoutID(1), 0, 5)
	sm.Add(LayoutID(1), 0, 10) // overwrite
...
func TestBuildPaintChunksEmpty(t *testing.T) {
	...
	chunks := BuildPaintChunks(dl, nil)
	if chunks.Len() != 0 {
...
func TestBuildPaintChunksSingleOwner(t *testing.T) {
	dl := NewDisplayCommandList()
	owner := LayoutID(1)
	dl.Add(makeRectCmd(RectF{X: 0, Y: 0, W: 10, H: 10}))
	dl.Add(makeRectCmd(RectF{X: 0, Y: 10, W: 10, H: 10}))
	dl.Add(makeRectCmd(RectF{X: 0, Y: 20, W: 10, H: 10}))

	owners := []LayoutID{owner, owner, owner}
	chunks := BuildPaintChunks(dl, owners)
...
func TestBuildPaintChunksMultipleOwners(t *testing.T) {
	...
	dl.Add(makeRectCmd(RectF{X: 0, Y: 0, W: 10, H: 10}))
	dl.Add(makeRectCmd(RectF{X: 0, Y: 10, W: 10, H: 10}))
	dl.Add(makeRectCmd(RectF{X: 20, Y: 0, W: 10, H: 10}))

	owners := []LayoutID{LayoutID(1), LayoutID(1), LayoutID(2)}
	chunks := BuildPaintChunks(dl, owners)

	if chunks.Len() != 2 {
		t.Fatalf("expected 2 chunks, got %d", chunks.Len())
	}
	if chunks.At(0).Owner != LayoutID(1) || chunks.At(0).CommandCount() != 2 {
		t.Errorf("chunk 0 = owner %d, count %d, want owner 1, count 2",
			chunks.At(0).Owner, chunks.At(0).CommandCount())
	}
	if chunks.At(1).Owner != LayoutID(2) || chunks.At(1).CommandCount() != 1 {
...
func TestBuildPaintChunksNilOwners(t *testing.T) {
	...
	if chunks.At(0).Owner != LayoutNone {
...
func TestBuildPaintChunksBoundsUnion(t *testing.T) {
	dl := NewDisplayCommandList()
	dl.Add(makeRectCmd(RectF{X: 0, Y: 0, W: 10, H: 10}))
	dl.Add(makeRectCmd(RectF{X: 20, Y: 30, W: 15, H: 5}))

	owners := []LayoutID{LayoutID(1), LayoutID(1)}
	chunks := BuildPaintChunks(dl, owners)
...
func TestBuildPaintChunksWithSourceMapping(t *testing.T) {
	...
	owners := []LayoutID{LayoutID(1), LayoutID(2)}
	chunks := BuildPaintChunks(dl, owners)

	sm := BuildSourceMapping(chunks)
	if sm.Len() != 2 {
...
func TestChunkedDisplayListNew(t *testing.T) {
	cdl := NewChunkedDisplayList(nil, NewPaintChunkList())
...
func TestChunkedDisplayListCommandsForChunk(t *testing.T) {
	...
	owners := []LayoutID{LayoutID(1), LayoutID(1), LayoutID(2)}
	chunks := BuildPaintChunks(dl, owners)
	cdl := NewChunkedDisplayList(dl, chunks)
...
func TestChunkedDisplayListInvalidateLayoutNone(t *testing.T) {
	...
	cdl.InvalidateByLayoutID(LayoutNone)
	if cdl.DirtyChunkCount() != 0 {
...
func TestChunkedDisplayListDirtyRects(t *testing.T) {
	dl := NewDisplayCommandList()
	dl.Add(makeRectCmd(RectF{X: 0, Y: 0, W: 10, H: 10}))
	dl.Add(makeRectCmd(RectF{X: 100, Y: 200, W: 50, H: 30}))
...
func TestChunkedDisplayListFindChunkByLayoutID(t *testing.T) {
	...
	idx, ok := cdl.FindChunkByLayoutID(LayoutID(2))
	if !ok || idx != 1 {
		t.Errorf("FindChunkByLayoutID(2) = (%d, %v), want (1, true)", idx, ok)
	}

	_, ok = cdl.FindChunkByLayoutID(LayoutID(99))
	if ok {
		t.Error("FindChunkByLayoutID(99) should return false")
	}

	_, ok = cdl.FindChunkByLayoutID(LayoutNone)
	if ok {
...
func TestChunkedDisplayListReuseUnchanged(t *testing.T) {
	...
	dl.Add(makeRectCmd(RectF{X: 0, Y: 0, W: 10, H: 10}))
	dl.Add(makeRectCmd(RectF{X: 20, Y: 0, W: 10, H: 10}))
	dl.Add(makeRectCmd(RectF{X: 40, Y: 0, W: 10, H: 10}))

	owners := []LayoutID{LayoutID(1), LayoutID(2), LayoutID(3)}
	chunks := BuildPaintChunks(dl, owners)
	cdl := NewChunkedDisplayList(dl, chunks)
...
func TestBuildPaintChunksNonContiguousSameOwner(t *testing.T) {
	...
	owners := []LayoutID{LayoutID(1), LayoutID(2), LayoutID(1)}
	chunks := BuildPaintChunks(dl, owners)
	...
	if chunks.At(0).Owner != LayoutID(1) || chunks.At(2).Owner != LayoutID(1) {
...
func TestBuildPaintChunksTextCommandBounds(t *testing.T) {
	...
	dl.Add(DisplayCommand{
		Kind: CmdText,
		Text: TextCommand{Bounds: RectF{X: 5, Y: 10, W: 100, H: 20}},
	})
...
func TestBuildPaintChunksImageCommandBounds(t *testing.T) {
	...
	dl.Add(DisplayCommand{
		Kind:  CmdImage,
		Image: ImageCommand{Bounds: RectF{X: 0, Y: 0, W: 200, H: 150}},
	})
...
func TestBuildPaintChunksPushPopCommandsNoBounds(t *testing.T) {
	dl := NewDisplayCommandList()
	dl.Add(DisplayCommand{Kind: CmdPushClip, Clip: ClipCommand{Bounds: RectF{X: 0, Y: 0, W: 100, H: 100}}})
	dl.Add(makeRectCmd(RectF{X: 10, Y: 10, W: 50, H: 50}))
	dl.Add(DisplayCommand{Kind: CmdPopClip})

	owners := []LayoutID{LayoutID(1), LayoutID(1), LayoutID(1)}
	chunks := BuildPaintChunks(dl, owners)
...
func makeRectCmd(r RectF) DisplayCommand {
	return DisplayCommand{
		Kind: CmdRect,
		Rect: RectCommand{Bounds: r, Color: color.RGBA{R: 255, A: 255}},
	}
}
...
func BenchmarkBuildPaintChunks(b *testing.B) {
	...
	for _, n := range sizes {
		b.Run(fmt.Sprintf("cmds=%d", n), func(b *testing.B) {
			dl := NewDisplayCommandList()
			owners := make([]LayoutID, n)
			for i := 0; i < n; i++ {
				dl.Add(makeRectCmd(RectF{X: float32(i), Y: 0, W: 10, H: 10}))
				owners[i] = LayoutID(i%10 + 1) // 10 distinct owners
			}
			b.ResetTimer()
			b.ReportAllocs()
			for i := 0; i < b.N; i++ {
				BuildPaintChunks(dl, owners)
			}
		})
	}
}
...
func BenchmarkChunkedDisplayListInvalidate(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		cdl.InvalidateByLayoutID(LayoutID(500))
		cdl.MarkAllClean()
	}
}
...
func BenchmarkChunkedDisplayListDirtyRects(b *testing.B) {
	...
	// Mark 10% dirty
	for i := 0; i < n; i += 10 {
		cdl.InvalidateByLayoutID(LayoutID(i + 1))
	}
...
func BenchmarkSourceMappingLookup(b *testing.B) {
	sm := NewSourceMapping()
	for i := 1; i <= 1000; i++ {
		sm.Add(LayoutID(i), i-1, i)
	}

	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		sm.Lookup(LayoutID(500))
	}
}

func BenchmarkPaintChunkContains(b *testing.B) {
	chunk := PaintChunk{Bounds: RectF{X: 0, Y: 0, W: 100, H: 100}}
	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
...
func BenchmarkPaintChunkIntersects(b *testing.B) {
	chunk := PaintChunk{Bounds: RectF{X: 0, Y: 0, W: 100, H: 100}}
	other := RectF{X: 50, Y: 50, W: 100, H: 100}
	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
