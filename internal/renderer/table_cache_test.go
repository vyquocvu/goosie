...
func TestTableColumnCacheEviction(t *testing.T) {
	cache := NewTableColumnCache(2) // capacity 2
...
func TestTableColumnCacheInvalidationIntegration(t *testing.T) {
	...
	// Create table tree
	table := NewRenderNode(NodeTypeElement)
	table.TagName = "table"

	row := NewRenderNode(NodeTypeElement)
	row.TagName = "tr"
	table.AddChild(row)

	cell := NewRenderNode(NodeTypeElement)
	cell.TagName = "td"
	row.AddChild(cell)
	...
	if _, ok := globalTableColumnCache.Get(table.ID, 800.0); !ok {
