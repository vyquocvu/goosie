...
func findNodeByID(node *RenderNode, id string) *RenderNode {
	if node == nil {
		return nil
	}
...
func TestMutationClassToggle(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, DisplayBlock, targetBox.Display)
	...
	le.nodeMapMu.RUnlock()
	require.True(t, ok)
	assert.Equal(t, DisplayFlex, targetBox2.Display)
}
...
func TestMutationAppendNode(t *testing.T) {
	...
	newParagraph := NewRenderNode(NodeTypeElement)
	...
	le.nodeMapMu.RUnlock()
	require.True(t, ok)
	assert.Equal(t, DisplayBlock, pBox.Display)
