...
func TestEvalCalcTerminatesOnMalformedInput(t *testing.T) {
	...
	for _, expr := range cases {
		...
		t.Run(expr, func(t *testing.T) {
			...
			go func() { done <- evalCalcExpr(expr, 16, 1280, 800, 800) }()
			select {
			case v := <-done:
				if v != v { // NaN
					t.Errorf("evalCalcExpr(%q) = NaN", expr)
				}
			case <-time.After(5 * time.Second):
				t.Fatalf("evalCalcExpr(%q) did not terminate (stack overflow regression)", expr)
			}
		})
	}
}
...
func TestEvalCalcDeepNestingTerminates(t *testing.T) {
	...
	go func() { done <- evalCalcExpr(deep, 16, 1280, 800, 800) }()
	select {
	case v := <-done:
		if v != v {
...
func TestEvalCalcBalancedValues(t *testing.T) {
	...
	for _, tc := range cases {
		if got := evalCalcExpr(tc.expr, tc.fontSize, 1280, 800, tc.pct); got != tc.want {
			t.Errorf("evalCalcExpr(%q) = %v, want %v", tc.expr, got, tc.want)
		}
	}
}
