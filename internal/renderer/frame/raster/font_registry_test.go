// Tests for the FontRegistry. The registry resolves a CSS font
...
func TestFontRegistry_GetSansSerifDefault(t *testing.T) {
	r := NewFontRegistry()
	face, ok := r.Get(FontDescriptor{Family: ""}, 16)
	...
	m12 := r.DesignMetrics(FontDescriptor{Family: ""}, 12)
	m24 := r.DesignMetrics(FontDescriptor{Family: ""}, 24)
...
func TestFontRegistry_AllFamilies(t *testing.T) {
	r := NewFontRegistry()
	for _, fam := range []string{FamilySansSerif, FamilySerif, FamilyMonospace} {
		fam := fam
		t.Run(fam, func(t *testing.T) {
			face, ok := r.Get(FontDescriptor{Family: fam}, 16)
...
func TestFontRegistry_BoldFaces(t *testing.T) {
	r := NewFontRegistry()
	reg, ok1 := r.Get(FontDescriptor{Family: FamilySansSerif}, 16)
	bold, ok2 := r.Get(FontDescriptor{Family: FamilySansSerif, Bold: true}, 16)
	require.True(t, ok1)
	require.True(t, ok2)
...
func TestFontRegistry_CacheReuse(t *testing.T) {
	r := NewFontRegistry()
	first, ok := r.Get(FontDescriptor{Family: FamilySansSerif}, 16)
	require.True(t, ok)
	second, ok := r.Get(FontDescriptor{Family: FamilySansSerif}, 16)
	require.True(t, ok)
...
func TestFontRegistry_ResolveFamily(t *testing.T) {
	cases := []struct {
		...
		{"", FamilySansSerif},
		{"sans-serif", FamilySansSerif},
		{"serif", FamilySerif},
		{"monospace", FamilyMonospace},
		{"Arial", FamilySansSerif},
		{"Helvetica", FamilySansSerif},
		{"HelveticaNeue", FamilySansSerif},
		{"Times", FamilySerif},
		{"Times New Roman", FamilySerif},
		{"Menlo", FamilyMonospace},
		{"Courier", FamilyMonospace},
		// Quoted family lists and falls-through generics.
		{`"Arial", sans-serif`, FamilySansSerif},
		{`'Courier', monospace`, FamilyMonospace},
		{`unknown, fantasy, serif`, FamilySerif},
	}
	for _, c := range cases {
		c := c
		t.Run(c.in, func(t *testing.T) {
			got := normaliseFamily(c.in)
			assert.Equal(t, c.want, got,
				"normaliseFamily(%q)", c.in)
		})
	}
}
...
func TestFontRegistry_DesignMetricsFallback(t *testing.T) {
	r := NewFontRegistry()
	m := r.DesignMetrics(FontDescriptor{Family: ""}, 20)
	...
	m10 := r.DesignMetrics(FontDescriptor{Family: ""}, 10)
...
func TestFontRegistry_SharedInstance(t *testing.T) {
	a := SharedFontRegistry()
	b := SharedFontRegistry()
	require.Same(t, a, b, "SharedFontRegistry must return the same instance")
}