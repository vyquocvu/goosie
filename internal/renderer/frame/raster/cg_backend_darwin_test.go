...
func TestCGBackendFillSolid(t *testing.T) {
	...
	cmds := []DisplayCmd{
		{Kind: CmdFill, Rect: frame.Rect{X: 10, Y: 10, W: 20, H: 20}, Color: frame.NewColor(255, 0, 0, 255)},
	}
...
func TestCGBackendFillWithDirtyRegion(t *testing.T) {
	...
	cmds := []DisplayCmd{
		{Kind: CmdFill, Rect: frame.Rect{X: 0, Y: 0, W: 100, H: 100}, Color: frame.NewColor(0, 255, 0, 255)},
	}
...
func TestCGBackendFillClipped(t *testing.T) {
	...
	cmds := []DisplayCmd{
		...
		{Kind: CmdFill, Rect: frame.Rect{X: 0, Y: 0, W: 100, H: 100}, Color: frame.NewColor(0, 0, 255, 255)},
		{Kind: CmdClipPop},
	}
	img, err := b.Rasterize(cmds, nil)
	if err != nil {
...
func TestCGBackendFillWithOpacity(t *testing.T) {
	...
	cmds := []DisplayCmd{
		...
		{Kind: CmdFill, Rect: frame.Rect{X: 0, Y: 0, W: 50, H: 50}, Color: frame.NewColor(255, 0, 0, 255)},
		{Kind: CmdOpacityPop},
	}
	img, err := b.Rasterize(cmds, nil)
	if err != nil {
...
func TestCGBackendBorderTop(t *testing.T) {
	...
	cmds := []DisplayCmd{
		{Kind: CmdBorder, Rect: frame.Rect{X: 10, Y: 10, W: 50, H: 50}, Border: BorderSpec{Top: SideSpec{Width: 3, Color: frame.NewColor(255, 0, 0, 255)}}},
	}
	img, err := b.Rasterize(cmds, nil)
	if err != nil {
		t.Fatal(err)
...
func TestCGBackendBorderAllSides(t *testing.T) {
	...
	cmds := []DisplayCmd{
		{Kind: CmdBorder, Rect: frame.Rect{X: 10, Y: 10, W: 50, H: 50}, Border: BorderSpec{
			Top: SideSpec{Width: 2, Color: red}, Right: SideSpec{Width: 2, Color: red},
			Bottom: SideSpec{Width: 2, Color: red}, Left: SideSpec{Width: 2, Color: red},
		}},
	}
	img, err := b.Rasterize(cmds, nil)
	if err != nil {
...
func TestCGBackendHiDPI(t *testing.T) {
	...
	cmds := []DisplayCmd{
		{Kind: CmdFill, Rect: frame.Rect{X: 0, Y: 0, W: 200, H: 200}, Color: frame.NewColor(128, 128, 128, 255)},
	}
	img, err := b.Rasterize(cmds, nil)
	if err != nil {
		t.Fatal(err)
...
func testBackendEquivalence(t *testing.T, name string, w, h int, cmds []DisplayCmd) {
	t.Helper()

	vp := frame.NewViewport(float32(w), float32(h), frame.PixelScaleDefault)

	cpuB := NewCPUBackend(w, h)
	cpuB.BeginFrame(vp)
	...
	cgB, err := NewCGBackend(w, h)
	if err != nil {
		t.Fatalf("NewCGBackend: %v", err)
	}
	defer cgB.Close()
	cgB.BeginFrame(vp)
...
func TestCGBackendVsCPUBackendFill(t *testing.T) {
	testBackendEquivalence(t, "cg_vs_cpu_fill", 200, 100, []DisplayCmd{
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 200, 100), Color: frame.White},
		{Kind: CmdFill, Rect: frame.NewRect(20, 20, 60, 60), Color: frame.NewColor(255, 0, 0, 255)},
	})
}

func TestCGBackendVsCPUBackendBorder(t *testing.T) {
	testBackendEquivalence(t, "cg_vs_cpu_border", 200, 100, []DisplayCmd{
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 200, 100), Color: frame.White},
		{Kind: CmdBorder, Rect: frame.NewRect(10, 10, 80, 80), Color: frame.Transparent, Border: BorderSpec{
			Top:    SideSpec{Width: 2, Color: frame.NewColor(0, 0, 255, 255)},
			Right:  SideSpec{Width: 2, Color: frame.NewColor(0, 0, 255, 255)},
			Bottom: SideSpec{Width: 2, Color: frame.NewColor(0, 0, 255, 255)},
			Left:   SideSpec{Width: 2, Color: frame.NewColor(0, 0, 255, 255)},
		}},
	})
}

func TestCGBackendVsCPUBackendClip(t *testing.T) {
	testBackendEquivalence(t, "cg_vs_cpu_clip", 200, 200, []DisplayCmd{
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 200, 200), Color: frame.White},
		{Kind: CmdClipPush, Rect: frame.NewRect(25, 25, 50, 50)},
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 200, 200), Color: frame.NewColor(0, 200, 0, 255)},
		{Kind: CmdClipPop},
	})
}

func TestCGBackendVsCPUBackendOpacity(t *testing.T) {
	testBackendEquivalence(t, "cg_vs_cpu_opacity", 200, 100, []DisplayCmd{
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 200, 100), Color: frame.NewColor(0, 0, 255, 255)},
		{Kind: CmdOpacityPush, Opacity: 0.5},
		{Kind: CmdFill, Rect: frame.NewRect(25, 0, 50, 100), Color: frame.NewColor(255, 0, 0, 255)},
		{Kind: CmdOpacityPop},
	})
}

func TestCGBackendVsCPUBackendNestedClip(t *testing.T) {
	testBackendEquivalence(t, "cg_vs_cpu_nested_clip", 200, 200, []DisplayCmd{
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 200, 200), Color: frame.White},
		{Kind: CmdClipPush, Rect: frame.NewRect(10, 10, 180, 180)},
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 200, 200), Color: frame.NewColor(200, 200, 200, 255)},
		{Kind: CmdClipPush, Rect: frame.NewRect(30, 30, 80, 80)},
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 200, 200), Color: frame.NewColor(100, 100, 255, 255)},
		{Kind: CmdClipPop},
		{Kind: CmdClipPop},
	})
}
...
func TestCGBackendVsCPUBackendComposite(t *testing.T) {
	testBackendEquivalence(t, "cg_vs_cpu_composite", 300, 200, []DisplayCmd{
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 300, 200), Color: frame.NewColor(240, 240, 240, 255)},
		{Kind: CmdFill, Rect: frame.NewRect(0, 0, 300, 40), Color: frame.NewColor(50, 50, 50, 255)},
		{Kind: CmdFill, Rect: frame.NewRect(0, 40, 80, 160), Color: frame.NewColor(200, 200, 200, 255)},
		{Kind: CmdFill, Rect: frame.NewRect(90, 50, 200, 30), Color: frame.NewColor(255, 255, 255, 255)},
		{Kind: CmdFill, Rect: frame.NewRect(90, 90, 200, 30), Color: frame.NewColor(255, 255, 255, 255)},
		{Kind: CmdFill, Rect: frame.NewRect(90, 130, 200, 30), Color: frame.NewColor(255, 255, 255, 255)},
		{Kind: CmdBorder, Rect: frame.NewRect(85, 45, 210, 120), Color: frame.Transparent, Border: BorderSpec{
			Top:    SideSpec{Width: 1, Color: frame.NewColor(180, 180, 180, 255)},
			Right:  SideSpec{Width: 1, Color: frame.NewColor(180, 180, 180, 255)},
			Bottom: SideSpec{Width: 1, Color: frame.NewColor(180, 180, 180, 255)},
			Left:   SideSpec{Width: 1, Color: frame.NewColor(180, 180, 180, 255)},
		}},
		{Kind: CmdFill, Rect: frame.NewRect(0, 190, 300, 10), Color: frame.NewColor(0, 120, 215, 255)},
	})
}
...
func BenchmarkCGFillRect100x100(b *testing.B) {
	backend, err := NewCGBackend(400, 400)
	if err != nil {
		b.Fatal(err)
	}
	defer backend.Close()
...
func BenchmarkCGFillRectFullFrame(b *testing.B) {
	backend, err := NewCGBackend(800, 600)
	if err != nil {
		b.Fatal(err)
	}
	defer backend.Close()
...
func BenchmarkCGBorderAllSides(b *testing.B) {
	...
	red := frame.NewColor(255, 0, 0, 255)
	cmds := []DisplayCmd{
		{Kind: CmdBorder, Rect: frame.Rect{X: 50, Y: 50, W: 200, H: 150}, Border: BorderSpec{
			Top: SideSpec{Width: 2, Color: red}, Right: SideSpec{Width: 2, Color: red},
			Bottom: SideSpec{Width: 2, Color: red}, Left: SideSpec{Width: 2, Color: red},
		}},
	}
