...
func TestBackendTypeString(t *testing.T) {
	tests := []struct {
		...
		{BackendUnspecified, "unspecified"},
		{BackendCPU, "cpu"},
		{BackendCoreGraphics, "core-graphics"},
	}
...
func TestSelectBackend(t *testing.T) {
	bt := SelectBackend()
	if bt != BackendCPU && bt != BackendCoreGraphics {
		t.Errorf("SelectBackend() = %v, want cpu or core-graphics", bt)
	}
	// Verify the selected backend can actually be constructed.
	b, _, err := NewBackend(1, 1)
	if err != nil {
		t.Fatalf("NewBackend with auto-select: %v", err)
	}
	b.Close()
}
...
func TestNewBackendDefault(t *testing.T) {
	b, bt, err := NewBackend(100, 100)
	if err != nil {
		t.Fatalf("NewBackend(100,100): %v", err)
	}
	defer b.Close()

	if bt != BackendCPU && bt != BackendCoreGraphics {
		t.Errorf("NewBackend returned BackendType = %v, want cpu or core-graphics", bt)
	}
...
func TestNewBackendZeroSize(t *testing.T) {
	b, _, err := NewBackend(0, 0)
	if err != nil {
		t.Fatalf("NewBackend(0,0): %v", err)
	}
	defer b.Close()
...
func TestNewBackendForceCPU(t *testing.T) {
	b, bt, err := NewBackend(100, 100, WithBackend(BackendCPU))
	if err != nil {
		t.Fatalf("NewBackend with WithBackend(CPU): %v", err)
	}
	defer b.Close()

	if bt != BackendCPU {
		t.Errorf("NewBackend with WithBackend(CPU) returned type = %v, want cpu", bt)
	}
	...
	cmds := []DisplayCmd{
		{Kind: CmdFill, Rect: frame.Rect{X: 0, Y: 0, W: 10, H: 10}, Color: frame.NewColor(0, 255, 0, 255)},
	}
	img, err := b.Rasterize(cmds, nil)
	if err != nil {
		t.Fatalf("Rasterize: %v", err)
...
func TestNewBackendForceCG(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("NewBackend with WithBackend(CG): %v", err)
	}
	defer b.Close()

	if bt != BackendCoreGraphics {
		t.Errorf("NewBackend with WithBackend(CG) returned type = %v, want core-graphics", bt)
	}
	...
	cmds := []DisplayCmd{
		{Kind: CmdFill, Rect: frame.Rect{X: 0, Y: 0, W: 50, H: 50}, Color: frame.NewColor(0, 0, 255, 255)},
	}
	img, err := b.Rasterize(cmds, nil)
	if err != nil {
		t.Fatalf("Rasterize: %v", err)
...
type crashBackend struct{ Backend }
...
func TestNewBackendCrashRecover(t *testing.T) {
	...
	// so we verify that the WithCrashRecover option is accepted and doesn't
	// break the normal path.
	b, bt, err := NewBackend(100, 100, WithCrashRecover())
	if err != nil {
		t.Fatalf("NewBackend with WithCrashRecover: %v", err)
	}
	defer b.Close()
...
func TestCPUAndCGProduceSameFill(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("NewBackend(CG): %v", err)
	}
	defer cgB.Close()

	cpuB, cpuBT, err := NewBackend(50, 50, WithBackend(BackendCPU))
	if err != nil {
		t.Fatalf("NewBackend(CPU): %v", err)
	}
	defer cpuB.Close()

	if cgBT != BackendCoreGraphics || cpuBT != BackendCPU {
		t.Fatalf("unexpected types: CG=%v CPU=%v", cgBT, cpuBT)
	}

	vp := frame.NewViewport(50, 50, frame.PixelScaleDefault)
	cmds := []DisplayCmd{
		{Kind: CmdFill, Rect: frame.Rect{X: 5, Y: 5, W: 40, H: 40}, Color: frame.NewColor(128, 200, 255, 255)},
	}
