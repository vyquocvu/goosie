...
func assertGolden(t *testing.T, name string, vp frame.Viewport, cmds []raster.DisplayCmd) {
	t.Helper()
	cfg := DefaultConfig()
	AssertGolden(t, name, cfg, vp, cmds)
}
...
func TestGoldenUpdateAndRegress(t *testing.T) {
	...
	AssertGolden(t, "roundtrip", cfg, vp, cmds)
...
func TestGoldenMismatchDetection(t *testing.T) {
	...
	imgA := toRGBA(backendA.FrameBuffer().Image())
	backendA.EndFrame()
	backendA.Close()
	...
	imgB := toRGBA(backendB.FrameBuffer().Image())
	backendB.EndFrame()
	backendB.Close()

	// They should not match.
	result := CompareImages(imgA, imgB, 0)
	if result.Match {
...
func TestGoldenTolerance(t *testing.T) {
	...
	// Create golden.
	os.Setenv("GOOSIE_UPDATE_GOLDEN", "1")
	AssertGolden(t, "tolerance_test", cfg, vp, cmds)
	os.Unsetenv("GOOSIE_UPDATE_GOLDEN")
