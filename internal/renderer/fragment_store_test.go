...
func TestFragmentIDNone(t *testing.T) {
	if FragmentNone != 0 {
		t.Errorf("FragmentNone should be 0, got %d", FragmentNone)
	}
}
...
func TestGetSetFragment(t *testing.T) {
	...
	// Set properties
	store.SetType(id, FragmentLine)
	store.SetBox(id, Rect{X: 10, Y: 20, Width: 100, Height: 50})

	frag = store.Get(id)
	if frag.Type != FragmentLine {
...
func TestGetInvalidFragmentID(t *testing.T) {
	store := NewFragmentStore(0)
	frag := store.Get(FragmentNone)
	if frag != nil {
		t.Error("Get(FragmentNone) should return nil")
	}
	frag = store.Get(FragmentID(999))
	if frag != nil {
...
func TestFragmentTypeTextRun(t *testing.T) {
	store := NewFragmentStore(0)
	id, _ := store.Allocate()
	store.SetType(id, FragmentTextRun)
	store.SetText(id, "Hello, World!")
	store.SetTextRange(id, 0, 13)

	frag := store.Get(id)
	if frag.Type != FragmentTextRun {
...
func TestFragmentTypeBox(t *testing.T) {
	store := NewFragmentStore(0)
	id, _ := store.Allocate()
	store.SetType(id, FragmentBox)
	store.SetLayoutID(id, LayoutID(42))

	frag := store.Get(id)
	if frag.Type != FragmentBox {
		t.Errorf("expected FragmentBox, got %d", frag.Type)
	}
	if frag.LayoutID != LayoutID(42) {
...
func TestFragmentTypeReplacedElement(t *testing.T) {
	store := NewFragmentStore(0)
	id, _ := store.Allocate()
	store.SetType(id, FragmentReplaced)
	store.SetIntrinsicSize(id, 100, 50)

	frag := store.Get(id)
	if frag.Type != FragmentReplaced {
...
func TestFragmentChain(t *testing.T) {
	...
	store.SetType(frag1, FragmentTextRun)
	store.SetType(frag2, FragmentTextRun)
	store.SetType(frag3, FragmentTextRun)
	...
	if store.NextFragment(frag3) != FragmentNone {
...
func TestSetTypeOnNone(t *testing.T) {
	store := NewFragmentStore(0)
	// Should not panic
	store.SetType(FragmentNone, FragmentLine)
}

func TestSetBoxOnNone(t *testing.T) {
	store := NewFragmentStore(0)
	// Should not panic
	store.SetBox(FragmentNone, Rect{})
}

func TestNextFragmentNone(t *testing.T) {
	store := NewFragmentStore(0)
	if store.NextFragment(FragmentNone) != FragmentNone {
...
func TestFirstFragmentNone(t *testing.T) {
	store := NewFragmentStore(0)
	if store.FirstFragment(LayoutNone) != FragmentNone {
...
func newTestFragmentStore() *FragmentStore {
	return NewFragmentStore(64)
}
...
func BenchmarkFragmentSetGet(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for _, id := range ids {
			...
			store.SetBox(id, Rect{X: 0, Y: 0, Width: 100, Height: 20})
			store.Get(id)
		}
	}
}
