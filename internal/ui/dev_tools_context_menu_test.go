...
func TestDevToolsContextMenu_Show_NoCrash(t *testing.T) {
	...
	menu := NewDevToolsContextMenu(DevToolsContextMenuOptions{
	...
	invoked := false
	menu2 := NewDevToolsContextMenu(DevToolsContextMenuOptions{
		OnInspect: func(n *renderer.RenderNode, _ *renderer.LayoutBox) {
			inspectedNode = n
			invoked = true
		},
...
func TestCSSSelector_PrefersID(t *testing.T) {
	...
	div.TagName = "div"
	div.SetAttribute("id", "root")
	assert.Equal(t, "#root", cssSelector(div))

	p := renderer.NewRenderNode(renderer.NodeTypeElement)
	p.TagName = "p"
	assert.Equal(t, "p", cssSelector(p))

	txt := renderer.NewRenderNode(renderer.NodeTypeText)
	txt.Text = "hello"
	assert.Equal(t, "", cssSelector(txt))
}
...
func TestRenderOuterHTML_TextOnly(t *testing.T) {
	...
	assert.Equal(t, "plain text only", renderOuterHTML(txt))
}
...
func TestEscapeAttr(t *testing.T) {
	...
	assert.Equal(t, `he said &quot;hi&quot;`, escapeAttr(`he said "hi"`))
	assert.Equal(t, "&lt;tag&gt;", escapeAttr("<tag>"))
}
...
func TestDevToolsContextMenu_Show_WithParent(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		var nilMenu *DevToolsContextMenu
