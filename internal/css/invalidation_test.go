...
func TestMutationCreation(t *testing.T) {
	...
	m := Mutation{
		Type:    MutationClassChange,
		Target:  elem,
		OldAttr: "container",
		NewAttr: "wrapper",
	}

	if m.Type != MutationClassChange {
...
func TestClassChangeInvalidatesMatchingRules(t *testing.T) {
	...
	result := inv.ComputeInvalidation(Mutation{
		Type:    MutationClassChange,
		Target:  elem,
		OldAttr: "highlight",
		NewAttr: "highlight",
...
func TestInlineStyleChangeInvalidatesSelf(t *testing.T) {
	...
	result := inv.ComputeInvalidation(Mutation{
		Type:   MutationInlineStyleChange,
		Target: elem,
	})
...
func TestAffectedRulesByClassChange(t *testing.T) {
	...
	affected := inv.affectedRuleIndices(Mutation{
		Type:    MutationClassChange,
		Target:  elem,
		OldAttr: "red",
		NewAttr: "red",
...
func TestDeclarationsContainInheritedProperty(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.css, func(t *testing.T) {
			...
			got := declarationsContainInherited(sheet.Rules[0].Declarations)
			if got != tt.inherited {
...
func makeTestStyleSheet(t *testing.T, cssText string) *CompiledStyleSheet {
	t.Helper()
	sheet := parseTestSheet(t, cssText)
	return CompileStyleSheet(sheet)
}

func parseTestSheet(t *testing.T, cssText string) *StyleSheet {
	t.Helper()
	p := NewParser(cssText)
	sheet, err := p.Parse()
	if err != nil {
...
func BenchmarkComputeInvalidation(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		inv.ComputeInvalidation(Mutation{
			Type:    MutationClassChange,
			Target:  elem,
			OldAttr: "highlight",
			NewAttr: "active",
...
func BenchmarkBatchMutations(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		inv.BeginBatch()
		inv.RecordMutation(Mutation{Type: MutationClassChange, Target: elem, OldAttr: "a", NewAttr: "a"})
		inv.RecordMutation(Mutation{Type: MutationClassChange, Target: elem, OldAttr: "b", NewAttr: "b"})
		inv.RecordMutation(Mutation{Type: MutationInlineStyleChange, Target: elem})
		inv.FlushBatch()
	}
}
...
func BenchmarkAffectedRuleIndices(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		inv.affectedRuleIndices(Mutation{
			Type:    MutationClassChange,
			Target:  elem,
			OldAttr: "container",
			NewAttr: "container",
...
func makeBenchStyleSheet(b *testing.B, cssText string) *CompiledStyleSheet {
	b.Helper()
	p := NewParser(cssText)
	sheet, err := p.Parse()
	if err != nil {
