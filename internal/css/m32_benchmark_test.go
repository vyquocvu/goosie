...
func BenchmarkComputeSpecificity(b *testing.B) {
	css := `div.container > p#intro.highlight:first-child { color: red; }`
	p := NewParser(css)
	sheet, _ := p.Parse()
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		ComputeSpecificity(&sheet.Rules[0].Selectors[0])
	}
}
...
func BenchmarkCompileStyleSheet(b *testing.B) {
	...
	b.Run("Small", func(b *testing.B) {
		p := NewParser(smallCSS)
		sheet, _ := p.Parse()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			CompileStyleSheet(sheet)
		}
	})

	b.Run("Medium", func(b *testing.B) {
		p := NewParser(mediumCSS)
		sheet, _ := p.Parse()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			CompileStyleSheet(sheet)
	...
	b.Run("SelectorComplex", func(b *testing.B) {
		p := NewParser(selectorComplexCSS)
		sheet, _ := p.Parse()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			CompileStyleSheet(sheet)
...
func linearMatchAll(rules []Rule, elem Element) int {
	count := 0
...
func linearMatchSequence(seq *SelectorSequence, elem Element) bool {
	return linearMatchFromRight(seq, elem)
}

func linearMatchFromRight(seq *SelectorSequence, elem Element) bool {
	if seq.Next == nil {
...
func linearMatchWithCombinator(seq *SelectorSequence, elem Element) bool {
...
func linearMatchSimple(sel *SimpleSelector, elem Element) bool {
