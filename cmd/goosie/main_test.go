...
func TestLoadURLCtxRejectsInvalidViewport(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			layer, _, _, err := loadURLCtx(context.Background(), client, fonts, u.String(), tc.w, tc.h, tc.scale)
			if err == nil {
...
func TestLoadURLCtxValidSmallDocument(t *testing.T) {
	...
	layer, _, _, err := loadURLCtx(context.Background(), client, fonts, u.String(), 800, 600, 1)
	if err != nil {
