...
// moduleRoot is the directory `go list ./v2/...` and a walk of v2 mean something in.
...
func packages(t *testing.T) []archtest.Package {
	t.Helper()
	pkgs, err := archtest.List(moduleRoot(t), "./v2/...")
	if err != nil {
...
func TestCgoConfinedToDarwinPlatform(t *testing.T) {
	...
	found, err := archtest.CheckCgo(v2Dir, darwinDir)
	if err != nil {
...
func TestRulesAreNotVacuous(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(moduleRoot(t), "v2", "internal", "frame", "grid.go")); err != nil {
