...
func TestNewSessionIsCreated(t *testing.T) {
	...
	if s.State() != session.StateCreated {
		t.Fatalf("initial state = %v, want %v", s.State(), session.StateCreated)
	}
...
func TestSessionNavigateTransitionsToNavigating(t *testing.T) {
	...
	if s.State() != session.StateNavigating {
		t.Fatalf("state = %v, want %v", s.State(), session.StateNavigating)
	}
	if s.NavID() != load.ID {
...
func TestSessionCancelTransitionsToCancelled(t *testing.T) {
	...
	if s.State() != session.StateCancelled {
		t.Fatalf("state after Cancel = %v, want %v", s.State(), session.StateCancelled)
	}
...
func TestSessionCompleteTransitionsToComplete(t *testing.T) {
	...
	if s.State() != session.StateComplete {
		t.Fatalf("state after Complete = %v, want %v", s.State(), session.StateComplete)
	}
}
...
func TestSessionFailTransitionsToFailed(t *testing.T) {
	...
	if s.State() != session.StateFailed {
		t.Fatalf("state after Fail = %v, want %v", s.State(), session.StateFailed)
	}
...
func TestSessionFailWithNilError(t *testing.T) {
	...
	if s.State() != session.StateFailed {
		t.Fatalf("state after Fail(nil) = %v, want %v", s.State(), session.StateFailed)
	}
	if s.NavigationErr() == nil {
...
func TestSessionCloseTransitionsToClosed(t *testing.T) {
	...
	if s.State() != session.StateClosed {
		t.Fatalf("state after Close = %v, want %v", s.State(), session.StateClosed)
	}
}
...
func TestSessionCloseIsIdempotent(t *testing.T) {
	...
	if s.State() != session.StateClosed {
		t.Fatalf("state = %v, want %v", s.State(), session.StateClosed)
	}
}
...
func TestSessionRepeatedNavigationAssignsNewIDs(t *testing.T) {
	...
	if s.State() != session.StateNavigating {
		t.Fatalf("state after second Navigate = %v, want %v", s.State(), session.StateNavigating)
	}
}
...
func TestSessionEventCallbackFiredOnNavigate(t *testing.T) {
	...
	var mu sync.Mutex
	s.SetEventCallback(func(ev session.Event) {
		mu.Lock()
	...
	if events[0].State != session.StateNavigating {
		t.Fatalf("event state = %v, want %v", events[0].State, session.StateNavigating)
	}
...
func TestSessionEventCallbackFiredOnAllTransitions(t *testing.T) {
	...
	mu.Lock()
	want := []session.State{session.StateNavigating, session.StateParsing, session.StateInteractive, session.StateComplete}
...
func TestSessionEventCallbackFiredOnCancel(t *testing.T) {
	...
	if got.State != session.StateCancelled {
		t.Fatalf("event state = %v, want %v", got.State, session.StateCancelled)
	}
	mu.Unlock()
}
...
func TestSessionEventCallbackFiredOnFail(t *testing.T) {
	...
	if stateEv.State != session.StateFailed {
		t.Fatalf("event state = %v, want %v", stateEv.State, session.StateFailed)
	}
	if !errors.Is(stateEv.Err, wantErr) {
		t.Fatalf("event Err = %v, want %v", stateEv.Err, wantErr)
	}
	if errEv.Type != session.EventError {
		t.Fatalf("event type = %v, want %v", errEv.Type, session.EventError)
	}
...
func TestSessionLifecycleFullSequence(t *testing.T) {
	...
	s.Parsing()
	if s.State() != session.StateParsing {
	...
	s.Interactive()
	if s.State() != session.StateInteractive {
...
func TestSessionStateStrings(t *testing.T) {
	tests := []struct {
		...
		{session.StateNavigating, "navigating"},
		{session.StateParsing, "parsing"},
		{session.StateInteractive, "interactive"},
		{session.StateComplete, "complete"},
		{session.StateCancelled, "cancelled"},
		{session.StateFailed, "failed"},
		{session.StateClosed, "closed"},
		{session.State(99), "state_99"},
	}
...
func TestSessionMultipleCompleteIsSafe(t *testing.T) {
	...
	s.Complete()
	if s.State() != session.StateComplete {
		t.Fatalf("state = %v, want %v", s.State(), session.StateComplete)
	}
}
...
func TestSessionMultipleCancelIsSafe(t *testing.T) {
	...
	s.Cancel()
	if s.State() != session.StateCancelled {
		t.Fatalf("state = %v, want %v", s.State(), session.StateCancelled)
	}
}
...
func TestFromContext(t *testing.T) {
	...
	rec := session.RecorderFromContext(ctx)
	if rec != nil {
...
func TestSessionEventQueueOldestDropped(t *testing.T) {
	...
	eventsReceived := make([]session.Event, 0)
	var mu sync.Mutex
...
func TestSessionEventImmutability(t *testing.T) {
	...
	for _, ev := range received {
		if ev.Type == session.EventSecuritySummary {
			foundSec = true
		...
		if ev.Type == session.EventDownload {
			foundDl = true
...
func TestSessionNewEvents(t *testing.T) {
	...
	events := make(map[session.EventType][]session.Event)
	var mu sync.Mutex
	s.SetEventCallback(func(ev session.Event) {
		mu.Lock()
	...
	if len(events[session.EventTitleChange]) != 1 || events[session.EventTitleChange][0].Title != "Hello World" {
		t.Fatalf("incorrect TitleChange event: %+v", events[session.EventTitleChange])
	}
	if len(events[session.EventURLChange]) != 1 || events[session.EventURLChange][0].URL != "https://example.com/new" {
		t.Fatalf("incorrect URLChange event: %+v", events[session.EventURLChange])
	}
	if len(events[session.EventFirstPaint]) != 1 {
		t.Fatalf("incorrect FirstPaint event: %+v", events[session.EventFirstPaint])
	}
	if len(events[session.EventProgress]) != 1 || events[session.EventProgress][0].Progress != 0.75 {
		t.Fatalf("incorrect Progress event: %+v", events[session.EventProgress])
	}
}
...
func navigateAndComplete(s *session.Session, url string) error {
...
func TestSessionNavigateFileURLFromRemoteOriginEmitsErrorEvent(t *testing.T) {
	...
	for _, ev := range events {
		if ev.Type == session.EventError && ev.Err == navigation.ErrFileAccessDenied {
			found = true
			break
		}
	}
