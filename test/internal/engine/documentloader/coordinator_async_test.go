...
func TestAsyncScriptFiresImmediately(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{
		Kind:       documentloader.KindScript,
		URL:        "async.js",
		ScriptMode: documentloader.ScriptModeAsync,
	})
	...
	if s.Scripts[0].Mode != documentloader.ScriptModeAsync {
...
func TestDeferScriptBuffersUntilDrain(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{
		Kind:       documentloader.KindScript,
		URL:        "defer.js",
		ScriptMode: documentloader.ScriptModeDefer,
	})
	...
	if s.Scripts[0].Mode != documentloader.ScriptModeDefer {
...
func TestModuleScriptStillSkipped(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{
		Kind:       documentloader.KindScript,
		URL:        "module.js",
		ScriptMode: documentloader.ScriptModeModule,
	})
...
func TestMixedModesOrder(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "classic.js"})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "async.js",
		ScriptMode: documentloader.ScriptModeAsync})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "defer.js",
		ScriptMode: documentloader.ScriptModeDefer})
	...
	if len(s.Scripts) != 1 || s.Scripts[0].Mode != documentloader.ScriptModeAsync {
	...
	wantOrder := []documentloader.ScriptMode{documentloader.ScriptModeAsync, documentloader.ScriptModeClassic, documentloader.ScriptModeDefer}
...
func TestLifecycleOrderAsyncAfterDrain(t *testing.T) {
	...
	if s.Lifecycle[0] != documentloader.EventDOMContentLoaded {
...
func TestLifecycleLoadWaitsForAsync(t *testing.T) {
	...
	if s.Lifecycle[1] != documentloader.EventLoad {
		t.Errorf("lifecycle[1] = %v, want Load", s.Lifecycle[1])
	}
	if s.Lifecycle[2] != documentloader.EventDocumentEnd {
...
func findSkip(errs []error, target **documentloader.SkippedError) bool {
	for _, e := range errs {
		var s *documentloader.SkippedError
		if asSkipped(e, &s) {
			*target = s
			return true
		}
...
func asSkipped(err error, target **documentloader.SkippedError) bool {
	if err == nil {
		return false
	}
	if s, ok := err.(*documentloader.SkippedError); ok {
		*target = s
		return true
	}
	return false
