...
type captureCallbacks struct {
	mu        sync.Mutex
	CSS       []documentloader.CSSResult
	Scripts   []documentloader.ScriptResult
	Images    []documentloader.ImageResult
	Errors    []error
	Lifecycle []documentloader.LifecycleEvent
}
...
func (c *captureCallbacks) record() documentloader.Callbacks {
	return documentloader.Callbacks{
		OnStylesheet: func(r documentloader.CSSResult) {
			c.mu.Lock()
			defer c.mu.Unlock()
			n := make([]documentloader.CSSResult, 0, len(c.CSS)+1)
			n = append(n, c.CSS...)
			n = append(n, r)
			c.CSS = n
		},
		OnScript: func(r documentloader.ScriptResult) {
			c.mu.Lock()
			defer c.mu.Unlock()
			n := make([]documentloader.ScriptResult, 0, len(c.Scripts)+1)
		...
		OnImage: func(r documentloader.ImageResult) {
			c.mu.Lock()
			defer c.mu.Unlock()
			n := make([]documentloader.ImageResult, 0, len(c.Images)+1)
			n = append(n, c.Images...)
			n = append(n, r)
			c.Images = n
		},
		OnError: func(_ documentloader.Resource, err error) {
			c.mu.Lock()
			defer c.mu.Unlock()
		...
		OnLifecycle: func(e documentloader.LifecycleEvent) {
			c.mu.Lock()
			defer c.mu.Unlock()
			n := make([]documentloader.LifecycleEvent, 0, len(c.Lifecycle)+1)
...
func copySliceCSS(s []documentloader.CSSResult) []documentloader.CSSResult {
	out := make([]documentloader.CSSResult, len(s))
	copy(out, s)
	return out
}
func copySliceScripts(s []documentloader.ScriptResult) []documentloader.ScriptResult {
	out := make([]documentloader.ScriptResult, len(s))
	copy(out, s)
	return out
}
func copySliceImages(s []documentloader.ImageResult) []documentloader.ImageResult {
	out := make([]documentloader.ImageResult, len(s))
	copy(out, s)
	return out
}
...
func copySliceLifecycle(s []documentloader.LifecycleEvent) []documentloader.LifecycleEvent {
	out := make([]documentloader.LifecycleEvent, len(s))
	copy(out, s)
	return out
}
...
func (h *testHarness) newCoord(t *testing.T, csp *net.CSPPolicy) *documentloader.Coordinator {
	t.Helper()
	c, err := documentloader.New(documentloader.Options{
...
func TestCSPRejectionPreventsFetch(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "https://evil.test/x.css"})

	// on-origin CSS: allowed; register and release.
	chCSS := h.fetcher.register("https://example.com/local.css")
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "local.css"})

	// off-origin script: skipped.
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "https://evil.test/x.js"})

	// on-origin script: allowed; register and release.
	chJS := h.fetcher.register("https://example.com/local.js")
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "local.js"})
	...
	for _, e := range h.cb.Errors {
		var skip *documentloader.SkippedError
...
func TestStylesheetSourceOrderWins(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "first.css"})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "second.css"})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "third.css"})
...
func TestMixedInlineExternalScriptsDocumentOrder(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{
		Kind: documentloader.KindScript, Inline: true, Source: []byte("inline-0"),
	})
	ch1 := h.fetcher.register("https://example.com/ext-1.js")
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "ext-1.js"})
	coord.HandleResource(documentloader.Resource{
		Kind: documentloader.KindScript, Inline: true, Source: []byte("inline-2"),
	})
	ch2 := h.fetcher.register("https://example.com/ext-3.js")
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "ext-3.js"})
...
func TestHandleResourceAfterClose(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "late.css"})
...
func TestInlineNonScriptRejected(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{
		Kind: documentloader.KindCSS, Inline: true, Source: []byte("body{}"),
	})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("HandleDocumentEnd: %v", err)
	}
...
func TestUnsupportedScriptModeSkipped(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{
		Kind: documentloader.KindScript, Inline: true, Source: []byte("x"), ScriptMode: documentloader.ScriptModeModule,
	})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("HandleDocumentEnd: %v", err)
	}
...
func TestNonHTTPSchemesSkipped(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "data:text/css,body{color:red}"})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "blob:https://x/abc"})
...
func TestSchedulerTracksResources(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "style.css"})
...
func TestLifecycleFiresAtDocumentEnd(t *testing.T) {
	...
	if h.cb.Lifecycle[0] != documentloader.EventDOMContentLoaded {
		t.Errorf("lifecycle[0] = %v, want EventDOMContentLoaded", h.cb.Lifecycle[0])
	}
	if h.cb.Lifecycle[1] != documentloader.EventLoad {
		t.Errorf("lifecycle[1] = %v, want EventLoad", h.cb.Lifecycle[1])
	}
	if h.cb.Lifecycle[2] != documentloader.EventDocumentEnd {
...
func TestNonHTTPDoesNotEnterScheduler(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "data:text/css,body{}"})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("HandleDocumentEnd: %v", err)
	}
...
func TestErrorContextIsInformative(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "x.css"})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "x.js"})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("HandleDocumentEnd: %v", err)
	}
	h.cb.snapshot()
...
func TestImageUsesImgSrcDirective(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindImage, URL: "https://example.com/img.png"})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("HandleDocumentEnd: %v", err)
	}
...
func TestEndToEndWithHTTPServer(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: "style.css"})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, URL: "script.js"})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindImage, URL: "img.png"})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("HandleDocumentEnd: %v", err)
	}
	cb.snapshot()
...
func TestNewRejectsMissingFields(t *testing.T) {
	...
	cases := []struct {
		name string
		opts documentloader.Options
	}{
		{"no context", documentloader.Options{Scheduler: sched, Fetcher: fetcher, FinalURL: "https://x/"}},
		{"no scheduler", documentloader.Options{NavigationContext: ctx, Fetcher: fetcher, FinalURL: "https://x/"}},
		{"no fetcher", documentloader.Options{NavigationContext: ctx, Scheduler: sched, FinalURL: "https://x/"}},
		{"no FinalURL", documentloader.Options{NavigationContext: ctx, Scheduler: sched, Fetcher: fetcher}},
		{"bad FinalURL", documentloader.Options{NavigationContext: ctx, Scheduler: sched, Fetcher: fetcher, FinalURL: "://malformed"}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := documentloader.New(tc.opts); err == nil {
