...
func TestFromDomResource_Mapping(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			want: documentloader.Resource{
				...
				URL:        "x.js",
				Position:   3,
				ScriptMode: documentloader.ScriptModeClassic,
			},
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := documentloader.FromDomResource(tc.in)
...
func TestFromDomResources_Batch(t *testing.T) {
	if got := documentloader.FromDomResources(nil); got != nil {
		t.Errorf("FromDomResources(nil) = %v, want nil", got)
	}
	out := documentloader.FromDomResources([]dom.Resource{
	...
	if out[0].Kind != documentloader.KindCSS || out[0].Position != 0 {
		t.Errorf("out[0] = %+v", out[0])
	}
	if out[1].ScriptMode != documentloader.ScriptModeDefer {
		t.Errorf("out[1].ScriptMode = %v, want ScriptModeDefer", out[1].ScriptMode)
	}
}

func TestFromDomResource_UnknownDefaults(t *testing.T) {
	got := documentloader.FromDomResource(dom.Resource{
		Kind:       dom.ResourceKind(99),
		URL:        "x",
		ScriptMode: dom.ScriptMode(99),
	})
	if got.Kind != documentloader.KindCSS {
		t.Errorf("unknown kind default = %v, want KindCSS", got.Kind)
	}
	if got.ScriptMode != documentloader.ScriptModeClassic {
...
func TestCoordinatorFromDomOnResourcePreservesSparsePositions(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindScript, Inline: true, Source: []byte("afterParser()")})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("HandleDocumentEnd: %v", err)
	}
	h.cb.snapshot()
...
func TestCoordinatorFromDomOnResource_ScriptModePreserved(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		if len(snapshot.Scripts) > 0 && snapshot.Scripts[0].Mode == documentloader.ScriptModeAsync {
			break
		}
	...
	seen := map[documentloader.ScriptMode]bool{}
	for _, script := range snapshot.Scripts {
		seen[script.Mode] = true
	}
	if !seen[documentloader.ScriptModeAsync] || !seen[documentloader.ScriptModeDefer] {
		t.Errorf("script modes = %v, want async and defer", seen)
	}
}

func resourceEqual(a, b documentloader.Resource) bool {
