...
func newSecondaryResourceFetcher(s *httptest.Server) documentloader.Fetcher {
...
func TestSecondaryResource_FontFace(t *testing.T) {
  ...
  src: url('myfont.woff2');
	...
	var (
		mu    sync.Mutex
		fonts []documentloader.FontResult
			...
			OnStylesheet: func(r documentloader.CSSResult) {
				...
				for _, sub := range extractFromSheet(sheet) {
					if sub.Kind == resourceFontKind {
						coord.EnqueueSecondary(context.Background(), documentloader.KindFont, sub.URL, r.Resolved)
					}
				}
			},
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: srv.base() + "/theme.css"})
	...
	mu.Lock()
	fontResults := append([]documentloader.FontResult(nil), fonts...)
	mu.Unlock()
...
func TestSecondaryResource_ImageInDeclaration(t *testing.T) {
	...
	var (
		mu   sync.Mutex
		imgs []documentloader.ImageResult
	)
	var coord *documentloader.Coordinator
			...
			OnStylesheet: func(r documentloader.CSSResult) {
				...
				for _, sub := range extractFromSheet(sheet) {
					if sub.Kind == resourceImageKind {
						coord.EnqueueSecondary(context.Background(), documentloader.KindImage, sub.URL, r.Resolved)
					}
				}
			},
			...
			OnStylesheet: func(r documentloader.CSSResult) {
				...
				for _, sub := range extractFromSheet(sheet) {
					if sub.Kind == resourceStylesheetKind {
						coord.EnqueueSecondary(context.Background(), documentloader.KindCSS, sub.URL, r.Resolved)
					}
				}
			},
		},
	})
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: srv.base() + "/dir/theme.css"})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("drain: %v", err)
	}
	if len(css) != 2 {
...
func TestMaxCSSImportDepth(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindCSS, URL: srv.base() + "/a.css"})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("drain: %v", err)
	}
...
func TestFontResult_HasSource(t *testing.T) {
	...
	coord.HandleResource(documentloader.Resource{Kind: documentloader.KindFont, URL: srv.base() + "/f.woff2"})
	if err := coord.HandleDocumentEnd(context.Background()); err != nil {
		t.Fatalf("drain: %v", err)
	}
