...
import (
	...
	"github.com/vyquocvu/goosie/internal/engine/renderer"
)

// helper: connect a Child and Parent over in-memory pipes.
func connect(t *testing.T) (*renderer.Child, *renderer.Parent, func()) {
	t.Helper()

	childInR, childInW := io.Pipe()
	childOutR, childOutW := io.Pipe()

	child := renderer.NewChild(childInR, childOutW)
	parent := renderer.NewParent(childInW, childOutR)
...
func TestProtocolRoundTrip(t *testing.T) {
	cmds := []*renderer.Command{
		renderer.NewNavigateCommand("https://example.com"),
		renderer.NewSetViewportCommand(800, 600),
		renderer.NewInputCommand(message.Input{
			Kind: message.InputClick,
			X:    100,
			Y:    200,
		}),
		renderer.NewPingCommand(),
		renderer.NewCloseCommand(),
	}
	...
	for _, cmd := range cmds {
		...
		got, err := renderer.DecodeCommand(data)
		if err != nil {
			t.Fatalf("Decode: %v", err)
		}
		if got.Version != renderer.ProtocolVersion {
			t.Errorf("Version = %d, want %d", got.Version, renderer.ProtocolVersion)
		}
	}
}
...
func TestNavigateProducesEvents(t *testing.T) {
	...
	if err := parent.Send(renderer.NewNavigateCommand("https://example.com")); err != nil {
		t.Fatal(err)
	}
	...
	// Send close
	if err := parent.Send(renderer.NewCloseCommand()); err != nil {
		t.Fatal(err)
	}
...
func TestPingPong(t *testing.T) {
	...
	if err := parent.Send(renderer.NewPingCommand()); err != nil {
		t.Fatal(err)
	}
...
func TestInvalidCommandReturnsError(t *testing.T) {
	...
	c := renderer.NewChild(strings.NewReader("{invalid json\n"), &buf)
...
func TestVersionMismatchRejectsCommand(t *testing.T) {
	cmd := &renderer.Command{
		Version:  999,
		Navigate: &renderer.NavigateCmd{URL: "https://example.com"},
	}
	data, err := cmd.Encode()
	if err != nil {
		t.Fatal(err)
	}
	_, err = renderer.DecodeCommand(data)
	if err == nil {
...
func TestChildWritesNewlineDelimited(t *testing.T) {
	var buf bytes.Buffer
	c := renderer.NewChild(strings.NewReader(""), &buf)

	renderer.WriteEvent(c, &message.Message{
		Version: message.Version,
	})
...
func TestParentSendConcurrent(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func() {
			defer wg.Done()
			parent.Send(renderer.NewPingCommand())
		}()
	}
	wg.Wait()
...
func TestViewportCommandOverIPC(t *testing.T) {
	...
	if err := parent.Send(renderer.NewSetViewportCommand(1024, 768)); err != nil {
		t.Fatal(err)
	}
...
func TestInputCommandOverIPC(t *testing.T) {
	...
	if err := parent.Send(renderer.NewInputCommand(message.Input{
		Kind: message.InputClick,
		X:    50,
		Y:    100,
	})); err != nil {
...
func TestNavigateAndViewportSequence(t *testing.T) {
	...
	if err := parent.Send(renderer.NewSetViewportCommand(1280, 720)); err != nil {
		t.Fatal(err)
	}
...
func TestChildPanicSendsCrashEvent(t *testing.T) {
	...
	c := renderer.NewChild(childInR, childOutW)
	...
	parent.Send(renderer.NewNavigateCommand("https://crash.example.com"))
...
func (p *panicOnNavigate) Run(ctx context.Context) error {
	defer func() {
		if r := recover(); r != nil {
			renderer.WriteEvent(p.Child, &message.Message{
				Version: message.Version,
				Time:    time.Now(),
				Crash: &message.Crash{
	...
	scanner := bufio.NewScanner(p.R().(io.Reader))
	scanner.Buffer(make([]byte, 0, 64*1024), 1024*1024)

	if scanner.Scan() {
		line := scanner.Bytes()
		cmd, err := renderer.DecodeCommand(line)
...
func TestTabCrashDetected(t *testing.T) {
	...
	var mu sync.Mutex
	var crashErr error
	tab := renderer.NewTab(child, parent)
...
func TestTabRestartAfterCrash(t *testing.T) {
	...
	child1 := renderer.NewChild(childInR1, childOutW1)
	parent1 := renderer.NewParent(childInW1, childOutR1)

	var mu sync.Mutex
	var crashDetected bool
	tab := renderer.NewTab(child1, parent1)
	tab.OnCrash = func(err error) {
		mu.Lock()
		crashDetected = true
		mu.Unlock()
	...
	child2 := renderer.NewChild(childInR2, childOutW2)
...
func BenchmarkNavigateOverIPC(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		parent.Start()
		go child.Run(context.Background())
		parent.Send(renderer.NewNavigateCommand("https://example.com"))
		time.Sleep(80 * time.Millisecond)
		parent.Send(renderer.NewCloseCommand())
...
func connect_b(b *testing.B) (*renderer.Child, *renderer.Parent, func()) {
	b.Helper()
	childInR, childInW := io.Pipe()
	childOutR, childOutW := io.Pipe()
	child := renderer.NewChild(childInR, childOutW)
