...
func TestScrollEventsCoalesceLatestWins(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		if err := loop.PostInput(eventloop.InputEvent{Type: eventloop.InputScroll, Viewport: eventloop.Viewport{Y: float32(i)}}); err != nil {
			t.Fatal(err)
		}
	}
...
func TestMouseMoveEventsCoalesceLatestWins(t *testing.T) {
	loop := eventloop.New(eventloop.Config{})
	for i := 0; i < 50; i++ {
		if err := loop.PostInput(eventloop.InputEvent{Type: eventloop.InputMouseMove, X: float32(i), Y: float32(i * 2)}); err != nil {
			t.Fatal(err)
		}
	}
	events := loop.DrainInput()
...
func TestClickAndKeyEventsPreserveOrdering(t *testing.T) {
	loop := eventloop.New(eventloop.Config{InputQueueSize: 4})
	want := []eventloop.InputEvent{
		{Type: eventloop.InputClick, Button: 1},
		{Type: eventloop.InputKey, Key: "a"},
		{Type: eventloop.InputClick, Button: 2},
		{Type: eventloop.InputKey, Key: "b"},
	}
	...
	if err := loop.PostInput(eventloop.InputEvent{Type: eventloop.InputClick}); !errors.Is(err, eventloop.ErrInputQueueFull) {
...
func TestRenderQueueKeepsLatestRequest(t *testing.T) {
	loop := eventloop.New(eventloop.Config{})
	first, err := loop.ScheduleRender(context.Background(), eventloop.RenderRequest{Generation: eventloop.Generation{Viewport: 1}})
	if err != nil {
		t.Fatal(err)
	}
	second, err := loop.ScheduleRender(context.Background(), eventloop.RenderRequest{Generation: eventloop.Generation{Viewport: 2}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCurrentRenderResultIsPresented(t *testing.T) {
	presented := make(chan eventloop.RenderResult, 1)
	gen := eventloop.Generation{Layout: 7}
	loop := eventloop.New(eventloop.Config{Present: func(result eventloop.RenderResult) { presented <- result }})
	loop.SetGeneration(gen)
	req, err := loop.ScheduleRender(context.Background(), eventloop.RenderRequest{Generation: gen})
	if err != nil {
		t.Fatal(err)
	}
	<-loop.RenderRequests()
	...
	if err := loop.SubmitRenderResult(eventloop.RenderResult{Request: req, Snapshot: "frame"}); err != nil {
		t.Fatal(err)
	}
	select {
...
func TestStaleRenderResultDroppedByGeneration(t *testing.T) {
	presented := make(chan eventloop.RenderResult, 1)
	loop := eventloop.New(eventloop.Config{Present: func(result eventloop.RenderResult) { presented <- result }})
	loop.SetGeneration(eventloop.Generation{Navigation: 2})
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	if err := loop.SubmitRenderResult(eventloop.RenderResult{Request: eventloop.RenderRequest{
		Context: ctx, Generation: eventloop.Generation{Navigation: 1},
	}}); err != nil {
		t.Fatal(err)
	}
	...
	go func() { done <- loop.Run(runCtx) }()
	waitForMetric(t, loop, func(m eventloop.Metrics) bool { return m.StaleFramesDropped == 1 })
	select {
	case <-presented:
...
func TestCancellationPreventsPresentation(t *testing.T) {
	...
	loop := eventloop.New(eventloop.Config{Present: func(eventloop.RenderResult) { presented.Add(1) }})
	gen := eventloop.Generation{Document: 1}
	loop.SetGeneration(gen)
	req, err := loop.ScheduleRender(context.Background(), eventloop.RenderRequest{Generation: gen})
	if err != nil {
		t.Fatal(err)
	...
	loop.RunCtx = runCtx
	loop.Mu.Unlock()
	loop.HandleRenderResult(eventloop.RenderResult{Request: req})
...
func TestMetricsCountCoalescedInputAndStaleFrames(t *testing.T) {
	loop := eventloop.New(eventloop.Config{})
	for i := 0; i < 3; i++ {
		_ = loop.PostInput(eventloop.InputEvent{Type: eventloop.InputScroll})
		_ = loop.PostInput(eventloop.InputEvent{Type: eventloop.InputMouseMove})
	}
	loop.SetGeneration(eventloop.Generation{DOM: 2})
	loop.HandleRenderResult(eventloop.RenderResult{Request: eventloop.RenderRequest{Generation: eventloop.Generation{DOM: 1}}})
	metrics := loop.Metrics()
...
func TestProcessPendingResultsNonBlocking(t *testing.T) {
	var presented atomic.Uint64
	gen := eventloop.Generation{Layout: 3}
	loop := eventloop.New(eventloop.Config{Present: func(eventloop.RenderResult) { presented.Add(1) }})
	loop.SetGeneration(gen)
...
func TestLoopShutsDownCleanly(t *testing.T) {
	...
	if err := loop.PostInput(eventloop.InputEvent{Type: eventloop.InputClick}); !errors.Is(err, eventloop.ErrClosed) {
		t.Fatalf("PostInput error = %v, want ErrClosed", err)
	}
}

func TestFrameBudget(t *testing.T) {
	budget := eventloop.NewFrameBudget(10 * time.Millisecond)
	start := time.Unix(0, 0)
...
func waitForMetric(t *testing.T, loop *eventloop.Loop, ready func(eventloop.Metrics) bool) {
	t.Helper()
