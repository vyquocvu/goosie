			...
			<main id="content" class="container fluid">
				...
				<section id="sec-2" class="section-block" data-category="archive">
					...
					<table id="data-table" class="table striped bordered" border="1" cellpadding="5">
						<thead>
							<tr><th>dom.ID</th><th>dom.Name</th><th>Status</th></tr>
						</thead>
						<tbody>
...
func TestAdversarial_ScriptMIMETypeFiltering(t *testing.T) {
	t.Run("MIMETypeAcceptanceMatrix", func(t *testing.T) {
		...
		for _, mime := range validJS {
			if !dom.IsJavaScriptMIMEType(mime) {
	...
	t.Run("MixedHTMLDocumentResourceDiscovery", func(t *testing.T) {
		...
		var scriptsDiscovered []dom.Resource
		_, err := dom.NewParser().ParseDocumentCtx(context.Background(), strings.NewReader(mixedHTML), dom.ParseConfig{
			OnResource: func(res dom.Resource) {
				if res.Kind == dom.ResourceScript {
		...
		if scriptsDiscovered[2].URL != "https://example.com/mod.js" || scriptsDiscovered[2].ScriptMode != dom.ScriptModeModule {
