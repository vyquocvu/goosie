...
func FuzzHTMLParseDocument(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, input string) {
		...
		if doc.Root == dom.NodeNone {
			t.Fatal("document root is NodeNone")
		}
		if kind := doc.Store.Kind(doc.Root); kind != dom.NodeKindDocument {
		...
		htmlID := dom.NodeNone
		for child := doc.Store.FirstChild(doc.Root); child != dom.NodeNone; child = doc.Store.NextSibling(child) {
			if doc.Store.Kind(child) == dom.NodeKindElement && doc.Store.Name(child) == atom.AtomHtml {
				htmlID = child
				break
			}
		}
		if htmlID == dom.NodeNone {
...
func FuzzHTMLGetElementByID(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, input, id string) {
		...
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		doc, perr := parser.ParseDocumentCtx(ctx, strings.NewReader(input), dom.ParseConfig{
			MaxBuf: maxFuzzInputBytes,
		})
		if perr != nil {
...
func walkForID(t *testing.T, store *dom.Store, root dom.NodeID, id string, found *bool) {
	t.Helper()
	var walk func(dom.NodeID)
	walk = func(n dom.NodeID) {
		if n == dom.NodeNone || *found {
			return
		}
		if store.Kind(n) == dom.NodeKindElement {
