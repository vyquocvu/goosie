...
func BenchmarkTableInternNew(b *testing.B) {
	...
	tbl := atom.NewTable(b.N+64, b.N*32+65536)
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkTableEviction(b *testing.B) {
	...
	const tableSize = 256
	tbl := atom.NewTable(tableSize, tableSize*32)
...
func BenchmarkStringDeduplication(b *testing.B) {
	...
	b.Run("AtomIntern", func(b *testing.B) {
		...
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			atoms := make([]atom.Atom, 0, len(classNames)*repeat)
			for r := 0; r < repeat; r++ {
				for _, cls := range classNames {
