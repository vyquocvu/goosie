...
func TestIsMainGoroutine_BeforeCaptureReturnsTrue(t *testing.T) {
	...
	// involved.
	prev := ui.MainGoroutineID.Load()
	ui.MainGoroutineID.Store(0)
	t.Cleanup(func() { ui.MainGoroutineID.Store(prev) })

	assert.True(t, ui.IsMainGoroutine(), "lazy capture should consider the caller as main")
}
...
func TestRunOnMainThread_NilNoop(t *testing.T) {
	assert.NotPanics(t, func() { ui.RunOnMainThread(nil) })
}
