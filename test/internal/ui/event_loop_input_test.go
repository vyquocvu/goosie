...
func newInputTestBrowser(t *testing.T, rec ui.HTMLRenderer) (*ui.Browser, *ui.Tab) {
	t.Helper()
	app := test.NewApp()
	t.Cleanup(app.Quit)

	w := app.NewWindow("test")
	browser := ui.NewBrowserInternal(app, w, true) // headless: do() runs inline
...
func TestTabEventLoop_MouseMoveBurstCoalescesToLatestHover(t *testing.T) {
	...
	browser.SetDevToolsVisible(true)
...
func TestTabEventLoop_LeftClickHitTestsWithScrollOffset(t *testing.T) {
	...
	browser, tab := newInputTestBrowser(t, rec)
	tab.EnsureHTMLRenderer()
	browser.SetDevToolsVisible(true) // Scroll first and let its drain apply the viewport, exactly as a real
...
func TestTabEventLoop_ClickAndKeyPreserveOrdering(t *testing.T) {
	...
	browser.SetDevToolsVisible(false)
	posted := []eventloop.InputEvent{
...
func TestTabEventLoop_MousePosterThrottlesHoverMoves(t *testing.T) {
	...
	tab.SetLastHoverHit(time.Now().Add(-time.Hour))
