...
func TestSourcesPanelVisual_Rendered(t *testing.T) {
	...
	p := devtools.NewSourcesPanel(func() *devtools.TabContext { return ctx })
	...
	p.Tree().Unselect(devtools.TreeResourceUID(cssIdx))
	p.Tree().Select(devtools.TreeResourceUID(scriptIdx))
	captureAndSave(t, w, p, "sources_panel_filtered.png")
}

func captureAndSave(t *testing.T, w fyne.Window, p *devtools.SourcesPanel, name string) {
	t.Helper()
	p.Refresh()
