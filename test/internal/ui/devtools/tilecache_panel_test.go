...
func TestTileCachePanel_InitialState(t *testing.T) {
	...
	p := devtools.NewTileCachePanelContent(func() *devtools.TabContext { return &devtools.TabContext{} })
	assert.NotNil(t, p)
}
...
func TestTileCachePanel_WithMemoryManager(t *testing.T) {
	...
	ctx := &devtools.TabContext{Memory: mgr}
	p := devtools.NewTileCachePanelContent(func() *devtools.TabContext { return ctx })
	assert.NotNil(t, p)
}
...
func TestTileCachePanel_FormatCount(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.want, func(t *testing.T) {
			assert.Equal(t, c.want, devtools.FormatCount(c.value, c.unit))
		})
	}
}
...
func TestTileCachePanel_FormatLimit(t *testing.T) {
	assert.Equal(t, "Image: unbounded", devtools.FormatLimit("Image", 0))
	assert.Equal(t, "Image: 1.0 KB", devtools.FormatLimit("Image", 1024))
	assert.Equal(t, "Image: 1.5 MB", devtools.FormatLimit("Image", 1024*1024+512*1024))
}
...
func TestTileCachePanel_FormatBudget(t *testing.T) {
	...
	out := devtools.FormatBudget(stats)
...
func TestTileCachePanel_SnapshotMetrics(t *testing.T) {
	// Nil recorder path.
	assert.Nil(t, devtools.SnapshotMetrics(&devtools.TabContext{}))

	// With a recorder (we use the mockRecorderProvider wired in
	// the devtools tests; see mock_metrics_provider_test.go).
	ctx := &devtools.TabContext{MetricsRecorder: &mockMetricsProvider{}}
	snap := devtools.SnapshotMetrics(ctx)
