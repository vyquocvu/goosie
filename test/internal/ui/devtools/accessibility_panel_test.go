...
func TestAccessibilityPanelEmpty(t *testing.T) {
	panel := devtools.NewAccessibilityPanel(nil)
	assert.NotNil(t, panel)
}

func TestAccessibilityPanel_NilTabContext(t *testing.T) {
	panel := devtools.NewAccessibilityPanel(nil).(*devtools.AccessibilityPanel)
...
func TestAccessibilityPanel_NilProvider(t *testing.T) {
	panel := devtools.NewAccessibilityPanel(nil).(*devtools.AccessibilityPanel)
	ctx := &devtools.TabContext{Accessibility: nil}
...
func TestAccessibilityPanel_RendersTree(t *testing.T) {
	...
	ctx := &devtools.TabContext{
		Accessibility: &mockA11yProvider{
			nodes: []*devtools.A11yNode{
				{
					Tag:  "nav",
					Role: "navigation",
					Name: "",
					Children: []*devtools.A11yNode{
						{Tag: "a", Role: "link", Name: "/home"},
						{Tag: "a", Role: "link", Name: "/about"},
					},
				},
				{Tag: "main", Role: "main", Children: []*devtools.A11yNode{
...
func TestA11yFormat(t *testing.T) {
	nodes := []*devtools.A11yNode{
		{Tag: "div", Role: "presentation", Children: []*devtools.A11yNode{
			{Tag: "button", Role: "button", Name: "Click me"},
		}},
	}
	formatted := devtools.FormatA11yTree(nodes, "")
...
func (m *mockA11yProvider) GetAccessibilityTree() []*devtools.A11yNode {
	return m.nodes
}
