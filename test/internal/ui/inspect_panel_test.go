...
func TestFormatCounterValue_RoundTrip(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			assert.Equal(t, c.want, ui.FormatCounterValue(c.e))
		})
	}
}
...
func TestElementsPanel_SyntaxHighlighting(t *testing.T) {
	...
	// Verify it's a ui.DOMTreeNodeWidget wrapping canvas.Text.
	nodeWidget, ok := obj.(*ui.DOMTreeNodeWidget)
	if !assert.True(t, ok, "tree node should be *ui.DOMTreeNodeWidget") {
		return
	}
...
// internals.
func countStylesObjects(panel *ui.InspectPanel) int {
