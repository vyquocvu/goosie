...
func TestPropertyInterning(t *testing.T) {
	cssText := `p { color: red; font-size: 16px; margin: 10px; }`
	...
	for _, decl := range rule.Declarations {
		...
		resolved := css.GetPropertyName(decl.PropertyAtom)
...
func TestHotColdClassification(t *testing.T) {
	cssText := `p { color: red; font-size: 16px; -webkit-transform: rotate(45deg); }`
	p := css.NewParser(cssText)
	sheet, err := p.Parse()
	if err != nil {
		t.Fatalf("Parse() failed: %v", err)
...
func TestOrigin(t *testing.T) {
	...
	if sheet.Rules[0].Origin != css.OriginAuthor {
		t.Errorf("expected OriginAuthor, got %d", sheet.Rules[0].Origin)
	}

	// Test custom origin
	config := css.ParseConfig{Origin: css.OriginUserAgent}
	p2 := css.NewParserWithConfig(cssText, config)
	sheet2, err := p2.Parse()
	if err != nil {
		t.Fatalf("Parse() failed: %v", err)
	}
	if sheet2.Rules[0].Origin != css.OriginUserAgent {
...
func TestMaxBytes(t *testing.T) {
	cssText := `p { color: red; } div { margin: 10px; }`

	// Parse with limit that cuts off second rule
	config := css.ParseConfig{MaxBytes: 20}
...
func TestBackwardCompatibility(t *testing.T) {
	cssText := `h1 { display: block; font-size: 32px; }`
	p := css.NewParser(cssText)
	sheet, err := p.Parse()
	if err != nil {
		t.Fatalf("Parse() failed: %v", err)
...
func TestParseStyleAttributeWithM31(t *testing.T) {
	decls, err := css.ParseStyleAttribute("color: red; font-size: 16px;")
	if err != nil {
