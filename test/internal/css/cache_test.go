// Package css_test — M9.2 tests for bounded selector and computed-style caches.
//
// Tests cover:
...
func (e *staticElem) ParentElement() css.Element {
	if e.parent == nil {
		return nil
	}
	return e.parent
}
func (e *staticElem) PreviousSiblingElement() css.Element    { return nil }
func (e *staticElem) ForEachChild(fn func(css.Element) bool) {}
func (e *staticElem) ForEachAncestor(fn func(css.Element) bool) {
	p := e.parent
	for p != nil {
		if !fn(p) {
			return
...
func (e *staticElem) ForEachPrecedingSibling(fn func(css.Element) bool) {}

// buildTestSheet returns a CompiledStyleSheet with a handful of rules.
func buildTestSheet() *css.CompiledStyleSheet {
	cssText := `
		#header { color: red; }
		.nav { display: flex; }
		div { margin: 0; }
...
func TestElementKey_FromElement(t *testing.T) {
	...
	key := css.ElementKeyFromElement(elem)
...
func TestElementKey_Equal(t *testing.T) {
	a := css.ElementKey{Tag: "div", ID: "x", ClassKey: "foo bar"}
	b := css.ElementKey{Tag: "div", ID: "x", ClassKey: "foo bar"}
	c := css.ElementKey{Tag: "div", ID: "y", ClassKey: "foo bar"}
...
func TestElementKey_Deterministic(t *testing.T) {
	...
	k1 := css.ElementKeyFromElement(elem)
	k2 := css.ElementKeyFromElement(elem)
	if k1 != k2 {
...
func TestMatchCache_HitAndMiss(t *testing.T) {
	...
	cache := css.NewMatchCache(css.MatchCacheConfig{MaxEntries: 64})
...
func TestMatchCache_Capacity(t *testing.T) {
	cache := css.NewMatchCache(css.MatchCacheConfig{MaxEntries: 3})

	// Insert 3 entries — fills to capacity.
	for i := 0; i < 3; i++ {
		key := css.ElementKey{Tag: "div", ID: fmt.Sprintf("id%d", i)}
		cache.Put(key, []css.CompiledRule{{SourceOrder: uint32(i)}})
	}
	if cache.Len() != 3 {
	...
	key4 := css.ElementKey{Tag: "p", ID: "id99"}
	cache.Put(key4, []css.CompiledRule{{SourceOrder: 99}})
...
func TestMatchCache_Update(t *testing.T) {
	cache := css.NewMatchCache(css.MatchCacheConfig{MaxEntries: 8})
	key := css.ElementKey{Tag: "div", ID: "foo"}

	cache.Put(key, []css.CompiledRule{{SourceOrder: 1}, {SourceOrder: 2}, {SourceOrder: 3}})
	cache.Put(key, []css.CompiledRule{{SourceOrder: 7}, {SourceOrder: 8}}) // update same key
...
func TestMatchCache_Clear(t *testing.T) {
	cache := css.NewMatchCache(css.MatchCacheConfig{MaxEntries: 8})
	for i := 0; i < 4; i++ {
		cache.Put(css.ElementKey{Tag: "div", ID: fmt.Sprintf("id%d", i)},
			[]css.CompiledRule{{SourceOrder: uint32(i)}})
	}
	cache.Clear()
...
func TestMatchCache_Race(t *testing.T) {
	cache := css.NewMatchCache(css.MatchCacheConfig{MaxEntries: 16})
	...
	for i := 0; i < 20; i++ {
		...
		go func(n int) {
			defer wg.Done()
			key := css.ElementKey{Tag: tags[n%len(tags)], ID: fmt.Sprintf("id%d", n%5)}
			_, ok := cache.Get(key)
			if !ok {
				cache.Put(key, []css.CompiledRule{{SourceOrder: uint32(n)}})
			}
		}(i)
	}
...
func TestStylePool_Evict(t *testing.T) {
	pool := css.NewStylePoolWithLimit(8)

	// Intern some distinct styles.
	for i := 0; i < 5; i++ {
		s := &css.InheritedStyle{FontSize: float32(10 + i)}
		pool.Intern(s)
	}
...
func TestStylePool_EvictPartial(t *testing.T) {
	pool := css.NewStylePoolWithLimit(16)
	for i := 0; i < 8; i++ {
		s := &css.InheritedStyle{FontSize: float32(i + 1)}
		pool.Intern(s)
	}
...
func TestStylePool_EvictRace(t *testing.T) {
	pool := css.NewStylePoolWithLimit(32)
	var wg sync.WaitGroup
	for i := 0; i < 20; i++ {
		wg.Add(1)
		go func(n int) {
			defer wg.Done()
			s := &css.InheritedStyle{FontSize: float32(n % 8)}
			pool.Intern(s)
		}(i)
	}
	wg.Add(1)
...
func BenchmarkMatchCache_Cached(b *testing.B) {
	sheet := buildTestSheet()
	cache := css.NewMatchCache(css.MatchCacheConfig{MaxEntries: 256})
	elem := &staticElem{tag: "div", id: "header", classes: []string{"container"}}
	key := css.ElementKeyFromElement(elem)
...
func BenchmarkMatchCache_ManyElements(b *testing.B) {
	...
	for _, tag := range tags {
		for _, id := range ids {
			...
			elems = append(elems, testElem{key: css.ElementKeyFromElement(e), element: e})
		}
	}
...
func BenchmarkStylePool_Evict(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		pool := css.NewStylePoolWithLimit(32)
		for j := 0; j < 16; j++ {
			s := &css.InheritedStyle{FontSize: float32(j + 1)}
			pool.Intern(s)
		}
		pool.Evict(^uint64(0))
	}
...
func TestMatchCacheAndStylePool_Integration(t *testing.T) {
	sheet := buildTestSheet()
	matchCache := css.NewMatchCache(css.MatchCacheConfig{MaxEntries: 32})
	stylePool := css.NewStylePoolWithLimit(32)
	...
	for _, e := range elems {
		key := css.ElementKeyFromElement(e)
		...
		s := &css.InheritedStyle{FontSize: 16, FontFamily: "sans-serif"}
...
func TestMatchCache_ZeroCapacity(t *testing.T) {
	...
	cache := css.NewMatchCache(css.MatchCacheConfig{MaxEntries: 0})
	key := css.ElementKey{Tag: "div"}
	cache.Put(key, []css.CompiledRule{{SourceOrder: 1}})
...
func TestMatchCache_LargeClassList(t *testing.T) {
	...
	key2 := css.ElementKeyFromElement(elem)
	if key != key2 {
