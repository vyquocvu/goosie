...
func TestComputeSpecificity(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			spec := css.ComputeSpecificity(&sheet.Rules[0].Selectors[0])
...
func TestCompareSpecificity(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := css.CompareSpecificity(tt.a, tt.b)
			if got != tt.want {
...
func TestCompiledMatchCompound(t *testing.T) {
	rawCSS := `p.highlight { background: yellow; }`
	p := css.NewParser(rawCSS)
	sheet, _ := p.Parse()
	compiled := css.CompileStyleSheet(sheet)
...
func TestCompiledMatchAttribute(t *testing.T) {
	rawCSS := `[type="text"] { border: 1px solid; }`
	p := css.NewParser(rawCSS)
	sheet, _ := p.Parse()
	compiled := css.CompileStyleSheet(sheet)
...
func TestCompileMultipleSelectorsPerRule(t *testing.T) {
	rawCSS := `h1, h2, h3 { font-family: Arial; }`
	p := css.NewParser(rawCSS)
	sheet, _ := p.Parse()
	compiled := css.CompileStyleSheet(sheet)
...
func (e *testElement) ParentElement() css.Element {
	if e.parent == nil {
		return nil
	}
	return e.parent
}
func (e *testElement) PreviousSiblingElement() css.Element {
	if e.prevSibling == nil {
		return nil
	}
	return e.prevSibling
}
func (e *testElement) ForEachChild(fn func(css.Element) bool) {
...
func (e *testElement) ForEachAncestor(fn func(css.Element) bool) {
...
func (e *testElement) ForEachPrecedingSibling(fn func(css.Element) bool) {
