...
func TestDisplayListLinks(t *testing.T) {
	...
	link := renderer.NewRenderNode(renderer.NodeTypeElement)
	link.TagName = "a"
	...
	layoutRoot := &renderer.LayoutBox{
		NodeID: root.ID,
		Box:    renderer.Rect{X: 0, Y: 0, Width: 100, Height: 20},
		Children: []*renderer.LayoutBox{
			{
				NodeID: link.ID,
				Box:    renderer.Rect{X: 0, Y: 0, Width: 80, Height: 20},
			},
		},
	}
