...
func TestImageLoadBatcher_CoalescesBurst(t *testing.T) {
	...
	b := renderer.NewImageLoadBatcher(10*time.Millisecond, func(srcs []string) {
		flushed <- srcs
	})
	defer b.Close()
...
func TestImageLoadBatcher_FlushForcesImmediate(t *testing.T) {
	flushed := make(chan []string, 1)
	b := renderer.NewImageLoadBatcher(time.Hour, func(srcs []string) {
		flushed <- srcs
	})
	defer b.Close()
...
func findFirstImg(n *renderer.RenderNode) *renderer.RenderNode {
	if n == nil {
		return nil
	}
	if n.TagName == "img" {
