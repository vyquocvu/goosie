...
func TestNewLayoutBox(t *testing.T) {
	nodeID := int64(123)
	lb := renderer.NewLayoutBox(nodeID)
	...
	if lb.Display != renderer.DisplayBlock {
...
func TestLayoutBoxAddChild(t *testing.T) {
	parent := renderer.NewLayoutBox(1)
	child := renderer.NewLayoutBox(2)
...
func TestLayoutBoxIsBlock(t *testing.T) {
	lb := renderer.NewLayoutBox(1)
	lb.Display = renderer.DisplayBlock
...
func TestLayoutBoxIsInline(t *testing.T) {
	lb := renderer.NewLayoutBox(1)
	lb.Display = renderer.DisplayInline
...
func TestLayoutBoxGetContentBox(t *testing.T) {
	lb := renderer.NewLayoutBox(1)
	lb.Box = renderer.Rect{X: 10, Y: 20, Width: 100, Height: 50}
	lb.PaddingTop = 5
	lb.PaddingRight = 10
	lb.PaddingBottom = 5
	lb.PaddingLeft = 10
...
func TestLayoutBoxMultipleChildren(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		child := renderer.NewLayoutBox(int64(i + 2))
		parent.AddChild(child)
	}
