...
func findNodeByTag(node *renderer.RenderNode, tagName string) *renderer.RenderNode {
...
func TestMediaQueryStyleApplication(t *testing.T) {
	...
	largeViewportSM := renderer.NewStyleManagerWithViewport(stylesheet, 800, 600)
	...
	renderTree2 := renderer.BuildRenderTree(findBodyNode(doc))
	if renderTree2 == nil {
		t.Fatal("BuildRenderTree returned nil")
	}

	// Test with small viewport (media query SHOULD apply)
	smallViewportSM := renderer.NewStyleManagerWithViewport(stylesheet, 500, 400)
...
func TestSetViewportDynamic(t *testing.T) {
	...
	stylesheet := extractAndParseCSS(doc)
	sm := renderer.NewStyleManagerWithViewport(stylesheet, 1024, 768)
