...
func TestCSSColorSupport(t *testing.T) {
	...
	var checkNode func(*renderer.RenderNode)
	checkNode = func(node *renderer.RenderNode) {
...
func TestCSSFontSizeSupport(t *testing.T) {
	...
	`
	_ = renderer.NewRenderer(800, 600)
	doc, err := html.Parse(strings.NewReader(htmlContent))
	if err != nil {
		t.Fatalf("html.Parse failed: %v", err)
	}
...
func TestImageFormatSupport(t *testing.T) {
	...
	// Count image nodes
	imageCount := 0
	var countImages func(*renderer.RenderNode)
	countImages = func(node *renderer.RenderNode) {
		if node.TagName == "img" {
			imageCount++
		}
		for _, child := range node.Children {
...
func TestLinkClickability(t *testing.T) {
	...
	linkFound := false
	var findLink func(*renderer.RenderNode)
	findLink = func(node *renderer.RenderNode) {
		if node.TagName == "a" {
			linkFound = true
