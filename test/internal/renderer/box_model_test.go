...
func TestParseLengthValues(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := renderer.ParseLength(tt.input, tt.fontSize)
...
func TestParseBoxShorthand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := renderer.ParseBoxShorthand(tt.input)
			if result != tt.expected {
				t.Errorf("renderer.ParseBoxShorthand(%q) = %v; want %v", tt.input, result, tt.expected)
			}
		})
	}
}
...
func findNodeByClass(node *renderer.RenderNode, className string) *renderer.RenderNode {
	if node == nil {
		return nil
	}
