...
func TestLayoutIDNone(t *testing.T) {
	if renderer.LayoutNone != 0 {
		t.Errorf("LayoutNone should be 0, got %d", renderer.LayoutNone)
	}
}

func TestLayoutIDValid(t *testing.T) {
	id := renderer.LayoutID(1)
	if !id.Valid() {
		t.Error("LayoutID(1) should be valid")
	}
	if renderer.LayoutNone.Valid() {
...
func TestNewLayoutStore(t *testing.T) {
	store := renderer.NewLayoutStore(0)
	if store == nil {
...
func TestNewLayoutStoreWithCapacity(t *testing.T) {
	store := renderer.NewLayoutStore(256)
	if store == nil {
		t.Fatal("NewLayoutStore returned nil")
	}
}
...
func TestAllocateMultiple(t *testing.T) {
	...
	seen := make(map[renderer.LayoutID]bool)
	for _, id := range ids {
		if seen[id] {
...
func TestGetInvalidID(t *testing.T) {
	store := renderer.NewLayoutStore(0)
	obj := store.Get(renderer.LayoutNone)
	if obj != nil {
		t.Error("Get(LayoutNone) should return nil")
	}
	obj = store.Get(renderer.LayoutID(999))
	if obj != nil {
...
func TestDisplayNoneNoAllocation(t *testing.T) {
	...
	childDOMID := int64(42)
	store.SetDOMToLayout(childDOMID, renderer.LayoutNone)
...
func TestDisplayNoneSkippedInTree(t *testing.T) {
	...
	store.SetDOMToLayout(2, renderer.LayoutNone)
...
func TestNextSibling(t *testing.T) {
	...
	if store.NextSibling(child3) != renderer.LayoutNone {
...
func TestLayoutStoreRemoveChild(t *testing.T) {
	...
	if store.FirstChild(parent) != renderer.LayoutNone {
...
func TestReset(t *testing.T) {
	...
	store.SetDOMToLayout(1, renderer.LayoutID(1))
	...
	if store.DOMToLayout(1) != renderer.LayoutNone {
...
func TestAppendChildToNone(t *testing.T) {
	store := renderer.NewLayoutStore(0)
	child, _ := store.Allocate()
	err := store.AppendChild(renderer.LayoutNone, child)
	if err == nil {
...
func TestSetDisplayOnNone(t *testing.T) {
	store := renderer.NewLayoutStore(0)
	// Should not panic
	store.SetDisplay(renderer.LayoutNone, "block")
}

func TestChildCountNone(t *testing.T) {
	store := renderer.NewLayoutStore(0)
	if store.ChildCount(renderer.LayoutNone) != 0 {
...
func newTestLayoutStore() *renderer.LayoutStore {
	return renderer.NewLayoutStore(64)
}
...
func BenchmarkLayoutStoreAllocate(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		store := renderer.NewLayoutStore(1024)
		for j := 0; j < 100; j++ {
			store.Allocate()
		}
	}
...
func BenchmarkLayoutStoreAppendChild(b *testing.B) {
	store := renderer.NewLayoutStore(1024)
	parent, _ := store.Allocate()
	children := make([]renderer.LayoutID, 100)
...
func BenchmarkLayoutStoreDOMMapping(b *testing.B) {
	store := renderer.NewLayoutStore(1024)
	ids := make([]renderer.LayoutID, 100)
	for i := range ids {
