...
func recordAt(c *renderer.FPSCounter, clock *fakeClock, dt time.Duration) {
	clock.advance(dt)
	c.RecordFrame()
}
...
func TestFPSCounterEmptySnapshot(t *testing.T) {
	...
	c := renderer.NewFPSCounterWithClock(clock.now)
...
func TestFPSFromInterval(t *testing.T) {
	assert.InDelta(t, 60.0, renderer.FPSFromInterval(time.Second/60), 0.01)
	assert.Zero(t, renderer.FPSFromInterval(0))
	assert.Zero(t, renderer.FPSFromInterval(-time.Millisecond))
}
