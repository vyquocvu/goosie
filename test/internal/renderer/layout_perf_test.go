...
func buildFlatDocumentTree(n int) *renderer.RenderNode {
	root := renderer.NewRenderNode(renderer.NodeTypeElement)
	root.TagName = "div"
	root.ID = 1

	for i := 0; i < n; i++ {
		para := renderer.NewRenderNode(renderer.NodeTypeElement)
		para.TagName = "p"
...
func buildDeepDocumentTree(depth int) (*renderer.RenderNode, []renderer.LayoutID) {
	tracker := renderer.NewReflowTracker()
	var leafIDs []renderer.LayoutID
	var nextID renderer.LayoutID = 1

	var build func(d int) renderer.LayoutID
	build = func(d int) renderer.LayoutID {
		id := nextID
		nextID++
		if d == 0 {
...
func TestM4PT1TextMutationLocalReflow(t *testing.T) {
	...
	rootID := renderer.LayoutID(1)
	sectionID := renderer.LayoutID(2)
	paraID := renderer.LayoutID(3)
	textID := renderer.LayoutID(4)
	...
	tracker.MarkDirty(textID, renderer.ReflowText)
...
func TestM4PT1TextMutationDoesNotEscapeBlock(t *testing.T) {
	...
	documentID := renderer.LayoutID(1)
	sectionID := renderer.LayoutID(2)
	paraID := renderer.LayoutID(3)
	textID := renderer.LayoutID(4)
...
func TestM4PT1DirtyParentEscalatesReflowRoot(t *testing.T) {
	...
	parentID := renderer.LayoutID(1)
	childID := renderer.LayoutID(2)

	tracker.SetParent(childID, parentID)

	// Parent has children dirty (e.g. a node was inserted).
	tracker.MarkDirty(parentID, renderer.ReflowChildren)
	tracker.MarkDirty(childID, renderer.ReflowText)
...
func TestM4PT1MultipleLeafMutations(t *testing.T) {
	...
	rootID := renderer.LayoutID(1)
	leftID := renderer.LayoutID(2)
	rightID := renderer.LayoutID(3)

	tracker.SetParent(leftID, rootID)
	tracker.SetParent(rightID, rootID)

	tracker.MarkDirty(leftID, renderer.ReflowText)
	tracker.MarkDirty(rightID, renderer.ReflowText)
...
func TestM4PT1IntrinsicSizeCachePreservedOnCleanNodes(t *testing.T) {
	...
	rootID := renderer.LayoutID(1)
	imgID := renderer.LayoutID(2)  // image: intrinsic size cached
	textID := renderer.LayoutID(3) // text node: will be mutated
...
func TestM4PT1CancellationSafe(t *testing.T) {
	tracker := renderer.NewReflowTracker()
	// Must not panic.
	tracker.MarkDirty(renderer.LayoutNone, renderer.ReflowText)
	tracker.MarkDirty(renderer.LayoutNone, renderer.ReflowGeometry)
	if tracker.IsDirty(renderer.LayoutNone) {
...
type layoutCallCounter struct {
	le    *renderer.LayoutEngine
	calls int
}

func (lc *layoutCallCounter) ComputeLayout(root *renderer.RenderNode) *renderer.LayoutBox {
	lc.calls++
...
func TestM4PT2ScrollDoesNotTriggerLayout(t *testing.T) {
	...
	le := &layoutCallCounter{le: renderer.NewLayoutEngine(800, 600)}
...
func BenchmarkM4PT1TextMutationReflowRoot(b *testing.B) {
	...
	for i := 1; i <= depth; i++ {
		tracker.SetParent(renderer.LayoutID(uint32(i)), renderer.LayoutID(uint32(i-1)))
	}
	leafID := renderer.LayoutID(depth)
	tracker.MarkDirty(leafID, renderer.ReflowText)
...
func BenchmarkM4PT2ScrollRenderWithViewport(b *testing.B) {
	...
	le := renderer.NewLayoutEngine(800, 3000)
	layoutRoot := le.ComputeLayout(root)
