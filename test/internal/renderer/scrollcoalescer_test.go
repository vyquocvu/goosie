...
func TestScrollCoalescer_CollapseAndClaim(t *testing.T) {
	...
	assert.True(t, c.Schedule(renderer.ScrollViewport{Y: 0, Height: 600}))
...
func TestScrollCoalescer_LastViewportWins(t *testing.T) {
	c := renderer.NewScrollCoalescer()
	assert.True(t, c.Schedule(renderer.ScrollViewport{Y: 0, Height: 600}))
	assert.False(t, c.Schedule(renderer.ScrollViewport{Y: 10, Height: 600}))
	assert.False(t, c.Schedule(renderer.ScrollViewport{Y: 20, Height: 600}))
...
func TestScrollCoalescer_CountsCoalesced(t *testing.T) {
	c := renderer.NewScrollCoalescer()
	c.Schedule(renderer.ScrollViewport{Y: 0, Height: 600})
	// The next four are coalesced; only the first Schedule increments.
	c.Schedule(renderer.ScrollViewport{Y: 1, Height: 600})
	c.Schedule(renderer.ScrollViewport{Y: 2, Height: 600})
	c.Schedule(renderer.ScrollViewport{Y: 3, Height: 600})
	c.Schedule(renderer.ScrollViewport{Y: 4, Height: 600})
	c.Schedule(renderer.ScrollViewport{Y: 5, Height: 600})
...
func TestFrameThrottler_AllowsFirstSignalImmediately(t *testing.T) {
	t0 := time.Unix(0, 0)
	th := renderer.NewFrameThrottler(16 * time.Millisecond)
