...
func TestMinContentSizeNoRecursionOnFormControlsWithText(t *testing.T) {
	...
	go func() {
		_, _, err := renderer.LayoutHTML(html, 1280, 800)
		if err != nil {
...
func TestMinContentSizeButtonTerminates(t *testing.T) {
	tree, _, err := renderer.LayoutHTML(`<!DOCTYPE html><html><body><button id="b">Sign in</button></body></html>`, 800, 600)
	if err != nil || tree == nil {
		t.Fatalf("LayoutHTML: %v", err)
	}
	var btn *renderer.RenderNode
	var find func(*renderer.RenderNode)
