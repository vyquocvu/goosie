...
func runLayout(t *testing.T, htmlStr string, cssStr string) (*renderer.LayoutEngine, *renderer.LayoutBox, *renderer.RenderNode) {
...
func TestIANAInlineLinksStayOnSameLineWithSurroundingText(t *testing.T) {
	...
	displayList := renderer.NewDisplayListBuilder().Build(layoutRoot, renderTree)

	var firstLink, secondLink *renderer.PaintCommand
	for _, cmd := range displayList.Commands {
		if cmd.Type != renderer.PaintLink {
			continue
		}
		switch cmd.LinkText {
		case "RFC 2606":
...
func TestTableCellInlineContentKeepsTableOffset(t *testing.T) {
	...
	displayList := renderer.NewDisplayListBuilder().Build(layoutRoot, renderTree)
	for _, cmd := range displayList.Commands {
		if cmd.Type == renderer.PaintLink && cmd.LinkText == "Domain Names" {
