...
func TestAdversarialTableMixedColumnConstraints(t *testing.T) {
	...
	renderRoot := renderer.BuildRenderTree(findBodyNode(doc))
	le := renderer.NewLayoutEngine(1000, 600)
	layoutRoot := le.ComputeLayout(renderRoot)
	...
	findTable = func(b *renderer.LayoutBox) {
		...
		if b.Display == renderer.DisplayGrid && len(b.Children) == 4 {
			tableBox = b
			return
		}
...
func TestAdversarialLayoutDeterminismStress(t *testing.T) {
	...
	renderRoot1 := renderer.BuildRenderTree(findBodyNode(doc))
	sm1 := renderer.NewStyleManager(extractAndParseCSS(doc))
	sm1.ApplyStyles(renderRoot1)
	layoutRoot1 := le.ComputeLayout(renderRoot1)
	dlb1 := renderer.NewDisplayListBuilder()
	...
	for iter := 1; iter <= 10; iter++ {
		...
		renderRootIter := renderer.BuildRenderTree(findBodyNode(docIter))
		smIter := renderer.NewStyleManager(extractAndParseCSS(docIter))
		smIter.ApplyStyles(renderRootIter)
		layoutRootIter := le.ComputeLayout(renderRootIter)
		dlbIter := renderer.NewDisplayListBuilder()
