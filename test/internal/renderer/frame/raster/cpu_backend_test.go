...
// ---------------------------------------------------------------------------
// raster.FrameBuffer tests
// ---------------------------------------------------------------------------

func TestNewFrameBuffer(t *testing.T) {
	fb := raster.NewFrameBuffer(100, 50)
	b := fb.Bounds()
	if b.Dx() != 100 || b.Dy() != 50 {
...
// ---------------------------------------------------------------------------
// raster.CPUBackend lifecycle tests
// ---------------------------------------------------------------------------

func TestCPUBackendBeginEndFrame(t *testing.T) {
	b := raster.NewCPUBackend(100, 100)
	defer b.Close()
	...
	cmds := []raster.DisplayCmd{
		{
			...
			Border: raster.BorderSpec{
				Top: raster.SideSpec{Width: 3, Color: frame.NewColor(255, 0, 0, 255)},
			},
		},
	}
	img, err := b.Rasterize(cmds, nil)
	...
	cmds := []raster.DisplayCmd{
		{
			...
			Rect: frame.Rect{X: 10, Y: 10, W: 50, H: 50},
			Border: raster.BorderSpec{
				Top:    raster.SideSpec{Width: 2, Color: red},
				Right:  raster.SideSpec{Width: 2, Color: red},
				Bottom: raster.SideSpec{Width: 2, Color: red},
				Left:   raster.SideSpec{Width: 2, Color: red},
			},
		},
	}
	img, err := b.Rasterize(cmds, nil)
...
func TestCPUBackendHiDPI(t *testing.T) {
	b := raster.NewCPUBackend(200, 200)
	defer b.Close()
...
func BenchmarkFillRect100x100(b *testing.B) {
	backend := raster.NewCPUBackend(400, 400)
...
func BenchmarkRasterText(b *testing.B) {
	...
	cmds := []raster.DisplayCmd{
		{Kind: raster.CmdText, Rect: frame.Rect{X: 50, Y: 50, W: 100, H: 100}, TextRun: textRun},
	}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func benchmarkFill(b *testing.B, w, h int) {
	backend := raster.NewCPUBackend(w, h)
...
func benchmarkText(b *testing.B, glyphCount int) {
	...
	cmds := []raster.DisplayCmd{
		{Kind: raster.CmdText, Rect: frame.Rect{X: 10, Y: 10, W: float32(glyphCount * 8), H: 20}, TextRun: textRun},
	}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
	...
	cmds := []raster.DisplayCmd{
		{
			...
			Image: raster.ImageSpec{Img: srcImg},
		},
	}
	b.ReportAllocs()
	b.ResetTimer()
	...
	cmds := []raster.DisplayCmd{
		{
			...
			Border: raster.BorderSpec{
				Top:    raster.SideSpec{Width: float32(4), Color: red},
				Right:  raster.SideSpec{Width: float32(4), Color: green},
				Bottom: raster.SideSpec{Width: float32(4), Color: blue},
				Left:   raster.SideSpec{Width: float32(4), Color: yellow},
			},
		},
	}
	b.ReportAllocs()
...
func BenchmarkRasterClipDepth(b *testing.B) {
	...
	vp := frame.NewViewport(800, 600, frame.PixelScaleDefault)
	backend.BeginFrame(vp)
	cmds := make([]raster.DisplayCmd, 0, 21)
	for i := 0; i < 10; i++ {
	...
	cmds = append(cmds, raster.DisplayCmd{
		Kind: raster.CmdFill, Color: frame.NewColor(0, 128, 255, 255),
		Rect: frame.Rect{X: 50, Y: 50, W: 700, H: 500},
	})
	for i := 0; i < 10; i++ {
		cmds = append(cmds, raster.DisplayCmd{Kind: raster.CmdClipPop})
	}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkRasterOpacityDepth(b *testing.B) {
	...
	cmds := make([]raster.DisplayCmd, 0, 21)
	for i := 0; i < 10; i++ {
		cmds = append(cmds, raster.DisplayCmd{Kind: raster.CmdOpacityPush, Opacity: 0.9})
	}
	cmds = append(cmds, raster.DisplayCmd{
		Kind: raster.CmdFill, Color: frame.NewColor(0, 128, 255, 255),
	...
	for i := 0; i < 10; i++ {
		cmds = append(cmds, raster.DisplayCmd{Kind: raster.CmdOpacityPop})
	}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkRasterMixedPage(b *testing.B) {
	...
	cmds := []raster.DisplayCmd{
		{Kind: raster.CmdFill, Rect: frame.Rect{X: 0, Y: 0, W: 800, H: 600}, Color: frame.NewColor(255, 255, 255, 255)},
		{Kind: raster.CmdFill, Rect: frame.Rect{X: 0, Y: 0, W: 800, H: 80}, Color: frame.NewColor(50, 50, 50, 255)},
		{Kind: raster.CmdBorder, Rect: frame.Rect{X: 0, Y: 0, W: 800, H: 80}, Border: raster.BorderSpec{Bottom: raster.SideSpec{Width: float32(2), Color: frame.NewColor(200, 200, 200, 255)}}},
		{Kind: raster.CmdText, Rect: frame.Rect{X: 20, Y: 10, W: 800, H: 60}, TextRun: frame.TextRun{Font: 0, FontSize: 24, Color: frame.NewColor(255, 255, 255, 255), Glyphs: glyphs[:10]}},
		{Kind: raster.CmdFill, Rect: frame.Rect{X: 0, Y: 80, W: 200, H: 440}, Color: frame.NewColor(240, 240, 240, 255)},
		{Kind: raster.CmdBorder, Rect: frame.Rect{X: 0, Y: 80, W: 200, H: 440}, Border: raster.BorderSpec{Right: raster.SideSpec{Width: float32(1), Color: frame.NewColor(200, 200, 200, 255)}}},
		{Kind: raster.CmdText, Rect: frame.Rect{X: 220, Y: 100, W: 560, H: 400}, TextRun: frame.TextRun{Font: 0, FontSize: 14, Color: frame.NewColor(0, 0, 0, 255), Glyphs: glyphs}},
		{Kind: raster.CmdImage, Rect: frame.Rect{X: 220, Y: 300, W: 200, H: 150}, Image: raster.ImageSpec{Img: srcImg}},
		{Kind: raster.CmdFill, Rect: frame.Rect{X: 0, Y: 520, W: 800, H: 80}, Color: frame.NewColor(50, 50, 50, 255)},
		{Kind: raster.CmdText, Rect: frame.Rect{X: 20, Y: 540, W: 760, H: 40}, TextRun: frame.TextRun{Font: 0, FontSize: 12, Color: frame.NewColor(200, 200, 200, 255), Glyphs: glyphs[:5]}},
	}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
