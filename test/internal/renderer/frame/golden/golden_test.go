...
func TestCompareImagesIdentical(t *testing.T) {
	...
	result := golden.CompareImages(img, img, 0)
	if !result.Match {
...
func TestCompareImagesDifferent(t *testing.T) {
	...
	result := golden.CompareImages(a, b, 0)
	if result.Match {
...
func TestCompareImagesWithinTolerance(t *testing.T) {
	...
	result := golden.CompareImages(a, b, 1)
	if !result.Match {
...
func TestCompareResultString(t *testing.T) {
	r := golden.CompareResult{Match: true, TotalPixels: 100, DiffPixels: 0, MaxDelta: 0, MeanDelta: 0}
	s := r.String()
	if s == "" {
...
func TestCompareResultPixelDiffRatio(t *testing.T) {
	r := golden.CompareResult{TotalPixels: 100, DiffPixels: 25}
...
func TestCompareResultPixelDiffRatioZero(t *testing.T) {
	r := golden.CompareResult{TotalPixels: 0}
...
func TestAssertGoldenUpdateMode(t *testing.T) {
	...
	golden.AssertGolden(t, "test_fill", cfg, vp, cmds)
...
func TestAssertGoldenMatch(t *testing.T) {
	...
	// First: create golden.
	os.Setenv("GOOSIE_UPDATE_GOLDEN", "1")
	golden.AssertGolden(t, "test_match", cfg, vp, cmds)
...
func TestCreateDiffImage(t *testing.T) {
	...
	diff := golden.CreateDiffImage(a, b, 0)
...
func BenchmarkCompareImages100x100(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		golden.CompareImages(img, img, 1)
	}
}
