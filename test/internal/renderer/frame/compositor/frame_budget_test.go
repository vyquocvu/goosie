...
// ---------------------------------------------------------------------------
// compositor.FrameBudget — defaults
// ---------------------------------------------------------------------------

func TestDefaultFrameBudget(t *testing.T) {
	b := compositor.DefaultFrameBudget()
	want := time.Second / 60
...
// ---------------------------------------------------------------------------
// compositor.FrameBudgetTracker — basic recording
// ---------------------------------------------------------------------------

func TestFrameBudgetTracker_RecordFrame(t *testing.T) {
	tr := compositor.NewFrameBudgetTracker(compositor.DefaultFrameBudget())
...
func TestFrameBudgetTracker_MissedFrame(t *testing.T) {
	budget := compositor.FrameBudget{TargetDuration: 16 * time.Millisecond, BudgetThreshold: 0.9}
	tr := compositor.NewFrameBudgetTracker(budget)
...
// ---------------------------------------------------------------------------
// compositor.FrameBudgetTracker — latency percentiles
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// compositor.FrameBudgetTracker — ring buffer overflow
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// compositor.FrameBudgetTracker — Reset
// ---------------------------------------------------------------------------
...
func TestFrameBudgetTracker_SanitizesConfig(t *testing.T) {
	tr := compositor.NewFrameBudgetTracker(compositor.FrameBudget{TargetDuration: -1, BudgetThreshold: 2.0})
	cfg := tr.Config()
...
// ---------------------------------------------------------------------------
// compositor.RasterJobQueue — basic operations
// ---------------------------------------------------------------------------

func TestRasterJobQueue_EnqueueDequeue(t *testing.T) {
	q := compositor.NewRasterJobQueue(10)

	job := compositor.RasterJob{Coord: compositor.TileCoord{Col: 1, Row: 2}, Generation: 1}
	if !q.Enqueue(job) {
...
func TestRasterJobQueue_Bounded(t *testing.T) {
	q := compositor.NewRasterJobQueue(2)
	q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: 0, Row: 0}})
	q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: 1, Row: 0}})

	if q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: 2, Row: 0}}) {
...
func TestRasterJobQueue_DequeueSkipsCancelled(t *testing.T) {
	...
	q.Enqueue(compositor.RasterJob{Coord: coord, Generation: 1})
	q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: 1, Row: 0}, Generation: 2})
	...
	if got.Coord != (compositor.TileCoord{Col: 1, Row: 0}) {
...
func TestRasterJobQueue_CancelCoord(t *testing.T) {
	q := compositor.NewRasterJobQueue(10)
	coord := compositor.TileCoord{Col: 3, Row: 4}
	q.Enqueue(compositor.RasterJob{Coord: coord})
	q.Enqueue(compositor.RasterJob{Coord: coord})
	q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: 0, Row: 0}})
...
func TestRasterJobQueue_CancelOutside(t *testing.T) {
	...
	q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: 0, Row: 0}})
	q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: 1, Row: 0}})
	q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: 2, Row: 0}})

	keep := map[compositor.TileCoord]bool{
		{Col: 0, Row: 0}: true,
	}
...
func TestRasterJobQueue_DefaultMax(t *testing.T) {
	q := compositor.NewRasterJobQueue(0)
	if q.Max != 256 {
...
func BenchmarkRasterJobQueue_EnqueueDequeue(b *testing.B) {
	q := compositor.NewRasterJobQueue(b.N + 1)
	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		q.Enqueue(compositor.RasterJob{Coord: compositor.TileCoord{Col: int32(i), Row: 0}})
	}
	for i := 0; i < b.N; i++ {
		q.Dequeue()
	}
