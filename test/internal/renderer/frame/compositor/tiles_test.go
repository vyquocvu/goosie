...
// ---------------------------------------------------------------------------
// compositor.Tile basics
// ---------------------------------------------------------------------------

func TestTileContains(t *testing.T) {
	tile := compositor.Tile{Bounds: frame.Rect{X: 100, Y: 200, W: 50, H: 50}}
	tests := []struct {
		name string
		pt   frame.Point
		want bool
...
func TestTileIntersects(t *testing.T) {
	tile := compositor.Tile{Bounds: frame.Rect{X: 0, Y: 0, W: 100, H: 100}}
	tests := []struct {
		name string
		r    frame.Rect
		want bool
...
// ---------------------------------------------------------------------------
// compositor.TileCache — construction and defaults
// ---------------------------------------------------------------------------

func TestDefaultTileCacheConfig(t *testing.T) {
	cfg := compositor.DefaultTileCacheConfig()
...
func TestNewTileCache_SanitizesConfig(t *testing.T) {
	c := compositor.NewTileCache(compositor.TileCacheConfig{TileWidth: -1, TileHeight: 0, MaxBytes: -5, MaxTiles: 0})
	cfg := c.Config()
	if cfg.TileWidth != 256 || cfg.TileHeight != 256 {
...
// ---------------------------------------------------------------------------
// compositor.TileCache — Get / Put
// ---------------------------------------------------------------------------

func TestTileCache_PutAndGet(t *testing.T) {
	c := compositor.NewTileCache(compositor.DefaultTileCacheConfig())
	coord := compositor.TileCoord{Col: 1, Row: 2}
...
func TestTileCache_PutUpdatesExisting(t *testing.T) {
	c := compositor.NewTileCache(compositor.DefaultTileCacheConfig())
	coord := compositor.TileCoord{Col: 0, Row: 0}

	t1 := &compositor.Tile{Coord: coord, Bounds: c.BoundsForCoord(coord), Version: 1, ByteSize: 100}
	c.Put(t1)

	t2 := &compositor.Tile{Coord: coord, Bounds: c.BoundsForCoord(coord), Version: 2, ByteSize: 200}
	c.Put(t2)
...
// ---------------------------------------------------------------------------
// compositor.TileCache — eviction by byte budget
// ---------------------------------------------------------------------------

func TestTileCache_EvictsByByteBudget(t *testing.T) {
	cfg := compositor.TileCacheConfig{TileWidth: 64, TileHeight: 64, MaxBytes: 200, MaxTiles: 100}
	c := compositor.NewTileCache(cfg)

	// Insert 3 tiles of 100 bytes each — only 2 fit in 200 byte budget.
	for i := int32(0); i < 3; i++ {
		coord := compositor.TileCoord{Col: i, Row: 0}
		c.Put(&compositor.Tile{Coord: coord, Bounds: c.BoundsForCoord(coord), ByteSize: 100})
	}
...
// ---------------------------------------------------------------------------
// compositor.TileCache — eviction by tile count
// ---------------------------------------------------------------------------

func TestTileCache_EvictsByTileCount(t *testing.T) {
	cfg := compositor.TileCacheConfig{TileWidth: 64, TileHeight: 64, MaxBytes: 1 << 30, MaxTiles: 3}
	c := compositor.NewTileCache(cfg)

	for i := int32(0); i < 5; i++ {
		coord := compositor.TileCoord{Col: i, Row: 0}
		c.Put(&compositor.Tile{Coord: coord, Bounds: c.BoundsForCoord(coord), ByteSize: 1})
	}
...
// ---------------------------------------------------------------------------
// compositor.TileCache — LRU eviction order
// ---------------------------------------------------------------------------

func TestTileCache_EvictsLRU(t *testing.T) {
	cfg := compositor.TileCacheConfig{TileWidth: 64, TileHeight: 64, MaxBytes: 1 << 30, MaxTiles: 2}
	c := compositor.NewTileCache(cfg)

	c0 := compositor.TileCoord{Col: 0, Row: 0}
	c1 := compositor.TileCoord{Col: 1, Row: 0}
	c2 := compositor.TileCoord{Col: 2, Row: 0}

	c.Put(&compositor.Tile{Coord: c0, Bounds: c.BoundsForCoord(c0), ByteSize: 1})
	c.Put(&compositor.Tile{Coord: c1, Bounds: c.BoundsForCoord(c1), ByteSize: 1})

	// Access c0 to make it more recent than c1.
	c.Get(c0)

	// Insert c2 — should evict c1 (least recently used).
	c.Put(&compositor.Tile{Coord: c2, Bounds: c.BoundsForCoord(c2), ByteSize: 1})
...
// ---------------------------------------------------------------------------
// compositor.TileCache — Invalidate
// ---------------------------------------------------------------------------

func TestTileCache_Invalidate(t *testing.T) {
	c := compositor.NewTileCache(compositor.DefaultTileCacheConfig())
	coord := compositor.TileCoord{Col: 1, Row: 1}
	c.Put(&compositor.Tile{Coord: coord, Bounds: c.BoundsForCoord(coord), ByteSize: 10})
...
func TestTileCache_InvalidateMissing(t *testing.T) {
	c := compositor.NewTileCache(compositor.DefaultTileCacheConfig())
	if c.Invalidate(compositor.TileCoord{Col: 99, Row: 99}) {
		t.Error("Invalidate should return false for missing tile")
	}
}

func TestTileCache_InvalidateRect(t *testing.T) {
	cfg := compositor.TileCacheConfig{TileWidth: 100, TileHeight: 100, MaxBytes: 1 << 30, MaxTiles: 100}
	c := compositor.NewTileCache(cfg)
	...
	for row := int32(0); row < 3; row++ {
		coord := compositor.TileCoord{Col: 0, Row: row}
		tile := c.Get(coord)
		if tile == nil || !tile.Dirty {
			t.Errorf("tile (0,%d) should be dirty", row)
		}
	}
	// compositor.Tile at (1,0) should NOT be dirty.
	tile := c.Get(compositor.TileCoord{Col: 1, Row: 0})
	if tile.Dirty {
...
// ---------------------------------------------------------------------------
// compositor.TileCache — Clear and Metrics
// ---------------------------------------------------------------------------

func TestTileCache_Clear(t *testing.T) {
	c := compositor.NewTileCache(compositor.DefaultTileCacheConfig())
	c.Put(&compositor.Tile{Coord: compositor.TileCoord{Col: 0, Row: 0}, ByteSize: 10})
	c.Get(compositor.TileCoord{Col: 0, Row: 0}) // hit
	c.Get(compositor.TileCoord{Col: 9, Row: 9}) // miss
...
func TestTileCache_MetricsTracking(t *testing.T) {
	c := compositor.NewTileCache(compositor.DefaultTileCacheConfig())
	coord := compositor.TileCoord{Col: 0, Row: 0}
	c.Put(&compositor.Tile{Coord: coord, ByteSize: 1})

	c.Get(coord)                     // hit
	c.Get(coord)                     // hit
	c.Get(compositor.TileCoord{Col: 5, Row: 5}) // miss
...
func TestCoordForPoint(t *testing.T) {
	c := compositor.NewTileCache(compositor.TileCacheConfig{TileWidth: 100, TileHeight: 50, MaxBytes: 1 << 20, MaxTiles: 100})
	tests := []struct {
		pt   frame.Point
		want compositor.TileCoord
	}{
		{frame.Point{X: 0, Y: 0}, compositor.TileCoord{Col: 0, Row: 0}},
		{frame.Point{X: 99, Y: 49}, compositor.TileCoord{Col: 0, Row: 0}},
		{frame.Point{X: 100, Y: 50}, compositor.TileCoord{Col: 1, Row: 1}},
		{frame.Point{X: 250, Y: 125}, compositor.TileCoord{Col: 2, Row: 2}},
	}
	for _, tt := range tests {
...
func TestBoundsForCoord(t *testing.T) {
	c := compositor.NewTileCache(compositor.TileCacheConfig{TileWidth: 100, TileHeight: 50, MaxBytes: 1 << 20, MaxTiles: 100})
	b := c.BoundsForCoord(compositor.TileCoord{Col: 2, Row: 3})
...
func TestCoordsInRect(t *testing.T) {
	c := compositor.NewTileCache(compositor.TileCacheConfig{TileWidth: 100, TileHeight: 100, MaxBytes: 1 << 20, MaxTiles: 100})

	// Rect spanning 2×2 tiles.
	coords := c.CoordsInRect(frame.Rect{X: 50, Y: 50, W: 150, H: 150})
	want := map[compositor.TileCoord]bool{
		{Col: 0, Row: 0}: true, {Col: 1, Row: 0}: true,
		{Col: 0, Row: 1}: true, {Col: 1, Row: 1}: true,
	}
...
func TestCoordsInRect_SingleTile(t *testing.T) {
	...
	if coords[0] != (compositor.TileCoord{Col: 0, Row: 0}) {
...
func TestPriorityForCoord(t *testing.T) {
	...
	// compositor.Tile (0,0) — fully visible.
	p := c.PriorityForCoord(compositor.TileCoord{Col: 0, Row: 0}, vp, 50)
	if p != compositor.TilePriorityVisible {
		t.Errorf("(0,0) priority = %d, want Visible", p)
	}

	// compositor.Tile (2,2) — bounds [200,200,300,300], visible [0,0,200,200].
	// Touching edge — not intersecting visible rect interior.
	// With 50px margin, expanded visible = [-50,-50,300,300] — should be Near.
	p = c.PriorityForCoord(compositor.TileCoord{Col: 2, Row: 2}, vp, 50)
	if p != compositor.TilePriorityNear {
		t.Errorf("(2,2) priority = %d, want Near", p)
	}

	// compositor.Tile (10,10) — far away, even with margin.
	p = c.PriorityForCoord(compositor.TileCoord{Col: 10, Row: 10}, vp, 50)
	if p != compositor.TilePriorityHidden {
...
func TestPriorityForCoord_WithScroll(t *testing.T) {
	...
	// compositor.Tile (5,5) — bounds [500,500,600,600] — visible.
	p := c.PriorityForCoord(compositor.TileCoord{Col: 5, Row: 5}, vp, 0)
	if p != compositor.TilePriorityVisible {
...
// ---------------------------------------------------------------------------
// compositor.TileCache — Evict (memory.Evictor interface)
// ---------------------------------------------------------------------------

func TestTileCache_Evict_FreesTargetBytes(t *testing.T) {
	cfg := compositor.TileCacheConfig{TileWidth: 64, TileHeight: 64, MaxBytes: 1 << 30, MaxTiles: 1000}
	c := compositor.NewTileCache(cfg)
...
func TestTileCache_Evict_All(t *testing.T) {
	...
	for i := int32(0); i < 3; i++ {
		coord := compositor.TileCoord{Col: i, Row: 0}
		c.Put(&compositor.Tile{Coord: coord, Bounds: c.BoundsForCoord(coord), ByteSize: 50})
	}
...
func TestTileCache_Evict_LRUOrder(t *testing.T) {
	cfg := compositor.TileCacheConfig{TileWidth: 64, TileHeight: 64, MaxBytes: 1 << 30, MaxTiles: 100}
	c := compositor.NewTileCache(cfg)
	...
	c.Put(&compositor.Tile{Coord: c0, Bounds: c.BoundsForCoord(c0), ByteSize: 100})
	c.Put(&compositor.Tile{Coord: c1, Bounds: c.BoundsForCoord(c1), ByteSize: 100})
	c.Put(&compositor.Tile{Coord: c2, Bounds: c.BoundsForCoord(c2), ByteSize: 100})
...
func BenchmarkTileCache_Put(b *testing.B) {
	c := compositor.NewTileCache(compositor.TileCacheConfig{TileWidth: 256, TileHeight: 256, MaxBytes: 1 << 30, MaxTiles: 10000})
	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		coord := compositor.TileCoord{Col: int32(i % 100), Row: int32(i / 100)}
		c.Put(&compositor.Tile{Coord: coord, ByteSize: 1})
	}
}
...
func BenchmarkTileCache_Get_Hit(b *testing.B) {
	...
	for i := int32(0); i < 100; i++ {
		c.Put(&compositor.Tile{Coord: compositor.TileCoord{Col: i, Row: 0}, ByteSize: 1})
	}
	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		c.Get(compositor.TileCoord{Col: int32(i % 100), Row: 0})
	}
}
...
func BenchmarkTileCache_Get_Miss(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		c.Get(compositor.TileCoord{Col: int32(i), Row: int32(i)})
	}
}
...
func BenchmarkTileCache_Evict(b *testing.B) {
	for i := 0; i < b.N; i++ {
		b.StopTimer()
		cfg := compositor.TileCacheConfig{TileWidth: 256, TileHeight: 256, MaxBytes: 1 << 30, MaxTiles: 10000}
		c := compositor.NewTileCache(cfg)
		for j := int32(0); j < 256; j++ {
			c.Put(&compositor.Tile{Coord: compositor.TileCoord{Col: j, Row: 0}, ByteSize: 4096})
		}
		b.StartTimer()
