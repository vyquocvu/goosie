...
// ---------------------------------------------------------------------------
// compositor.SceneSnapshot — immutability and staleness
// ---------------------------------------------------------------------------

func TestSceneSnapshot_IsStale(t *testing.T) {
	snap := compositor.SceneSnapshot{Generation: 5}
	if snap.IsStale(5) {
...
func TestSceneSnapshot_FindTile(t *testing.T) {
	snap := compositor.SceneSnapshot{
		Tiles: []compositor.SnapshotTile{
			{Coord: compositor.TileCoord{Col: 0, Row: 0}, Version: 1},
			{Coord: compositor.TileCoord{Col: 1, Row: 0}, Version: 2},
			{Coord: compositor.TileCoord{Col: 0, Row: 1}, Version: 3},
		},
	}

	found := snap.FindTile(compositor.TileCoord{Col: 1, Row: 0})
	if found == nil {
	...
	missing := snap.FindTile(compositor.TileCoord{Col: 5, Row: 5})
	if missing != nil {
...
func TestSceneSnapshot_VisibleTiles(t *testing.T) {
	snap := compositor.SceneSnapshot{
		...
		Tiles: []compositor.SnapshotTile{
			{Coord: compositor.TileCoord{Col: 0, Row: 0}, Bounds: frame.Rect{X: 0, Y: 0, W: 100, H: 100}},
			{Coord: compositor.TileCoord{Col: 1, Row: 0}, Bounds: frame.Rect{X: 100, Y: 0, W: 100, H: 100}},
			{Coord: compositor.TileCoord{Col: 0, Row: 1}, Bounds: frame.Rect{X: 0, Y: 100, W: 100, H: 100}},
			{Coord: compositor.TileCoord{Col: 5, Row: 5}, Bounds: frame.Rect{X: 500, Y: 500, W: 100, H: 100}},
		},
	}
	...
	if visible[0].Coord != (compositor.TileCoord{Col: 0, Row: 0}) {
...
func TestSceneSnapshot_VisibleTilesWithScroll(t *testing.T) {
	...
	if visible[0].Coord != (compositor.TileCoord{Col: 1, Row: 0}) {
		t.Errorf("visible tile = %v, want (1,0)", visible[0].Coord)
	}
}

// ---------------------------------------------------------------------------
// compositor.SnapshotPublisher — publish and generation tracking
// ---------------------------------------------------------------------------
...
func TestSnapshotPublisher_PublishBumpsGeneration(t *testing.T) {
	...
	pub := compositor.NewSnapshotPublisher(cache)
...
func BenchmarkSnapshotPublisher_Publish(b *testing.B) {
	...
	for col := int32(0); col < 16; col++ {
		for row := int32(0); row < 16; row++ {
			...
			cache.Put(&compositor.Tile{Coord: coord, Bounds: cache.BoundsForCoord(coord), ByteSize: 1, Version: 1})
		}
	}
	pub := compositor.NewSnapshotPublisher(cache)
...
func BenchmarkSceneSnapshot_VisibleTiles(b *testing.B) {
	tiles := make([]compositor.SnapshotTile, 256)
	for i := range tiles {
		tiles[i] = compositor.SnapshotTile{
			Coord:  compositor.TileCoord{Col: int32(i % 16), Row: int32(i / 16)},
...
func BenchmarkSceneSnapshot_FindTile(b *testing.B) {
	tiles := make([]compositor.SnapshotTile, 256)
	for i := range tiles {
		tiles[i] = compositor.SnapshotTile{Coord: compositor.TileCoord{Col: int32(i % 16), Row: int32(i / 16)}}
	}
	snap := compositor.SceneSnapshot{Tiles: tiles}
	target := compositor.TileCoord{Col: 15, Row: 15} // worst case
	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
