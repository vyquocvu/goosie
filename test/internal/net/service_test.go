...
func TestServiceFetchRecordsRequestLog(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: client, UserAgent: "Goosie-Test"})
...
func TestServiceDefaultClientHasCookieJar(t *testing.T) {
	service := net.NewService(net.ServiceOptions{})
...
func TestLimitedContextReaderReadsNormally(t *testing.T) {
	body := "hello, world"
	reader := net.NewLimitedContextReader(context.Background(), strings.NewReader(body), 100)
...
func TestLimitedContextReaderExceedsLimit(t *testing.T) {
	reader := net.NewLimitedContextReader(
		context.Background(),
		strings.NewReader("this body is too long"),
		5,
	)
	...
	if !errors.Is(err, net.ErrBodyTooLarge) {
...
func TestServiceFetchWithContextBodyTooLarge(t *testing.T) {
	...
	if !errors.Is(err, net.ErrBodyTooLarge) && !strings.Contains(err.Error(), "body too large") {
		t.Fatalf("error = %v, want ErrBodyTooLarge", err)
	}
}
...
func TestServiceFetchWithContextCancelledDuringBodyRead(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: &http.Client{}})
...
func TestServiceFetchWithContextBodySizePreservesProgressCallback(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: client, MaxBodySize: 100})
...
func TestServiceFetchWithContextBodySizeDoesNotAffectCache(t *testing.T) {
	...
	cache := net.NewHTTPCache(t.TempDir(), false)
	service := net.NewService(net.ServiceOptions{Client: client, Cache: cache, MaxBodySize: 100})
...
func TestServiceFetchWithContextBodySizeUnderLimitWithBinaryData(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: client, MaxBodySize: 200})
...
func TestServiceFetchWithContextBodySizeOneByteOverLimit(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: client, MaxBodySize: 10})
...
func TestServiceFetchWithContextBodySizeZeroLimitWithErrorResponse(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: client, MaxBodySize: 0})
...
func BenchmarkLimitedContextReader(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		reader := net.NewLimitedContextReader(ctx, strings.NewReader(body), 0)
		_, err := io.ReadAll(reader)
		if err != nil {
...
func BenchmarkLimitedContextReaderWithLimit(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		reader := net.NewLimitedContextReader(ctx, strings.NewReader(body), 10000)
		_, err := io.ReadAll(reader)
		if err == nil {
...
func TestServiceRedirectedResponseDoesNotPopulateOriginalURLCache(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: client, Cache: net.NewHTTPCache(t.TempDir(), false)})
...
func TestFetchStreamBodyTooLarge(t *testing.T) {
	...
	if !errors.Is(readErr, net.ErrBodyTooLarge) {
...
func TestFetchWithMetaRedirectLimitExceeded(t *testing.T) {
	...
	server := redirectServer(net.MaxRedirects+10, "should not reach")
	defer server.Close()
	...
	if meta.RedirectCount != net.MaxRedirects {
		t.Errorf("RedirectCount = %d, want %d", meta.RedirectCount, net.MaxRedirects)
	}
...
func TestNewServiceAppliesDefaultMaxBodySize(t *testing.T) {
	service := net.NewService(net.ServiceOptions{})
	if service.MaxBodySize() != net.DefaultMaxBodySize {
		t.Fatalf("maxBodySize = %d, want %d", service.MaxBodySize(), net.DefaultMaxBodySize)
	}
}

func TestNewServiceExplicitMaxBodySize(t *testing.T) {
	service := net.NewService(net.ServiceOptions{MaxBodySize: 42})
...
func TestNewServiceNegativeMaxBodySizeDisablesLimit(t *testing.T) {
	service := net.NewService(net.ServiceOptions{MaxBodySize: -1})
...
func TestContentLengthPreCheckRejectsOversizedResponse(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: client, MaxBodySize: 100 * 1024 * 1024})
...
func TestContentLengthPreCheckSkipsWhenUnlimited(t *testing.T) {
	...
	service := net.NewService(net.ServiceOptions{Client: client, MaxBodySize: -1})
...
func TestDefaultMaxBodySizeRejectsLargeResponse(t *testing.T) {
	...
	largeBody := strings.Repeat("x", int(net.DefaultMaxBodySize)+1)
	client := &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
...
func TestDecompressionBombDetection(t *testing.T) {
	...
	decompressedSize := int64(1000*net.MaxDecompressionRatio + 1) // exceeds ratio
	...
	if !errors.Is(err, net.ErrDecompressedTooLarge) {
...
func TestDecompressionBombWithFetchStream(t *testing.T) {
	...
	decompressedSize := int64(100*net.MaxDecompressionRatio + 1)
	body := strings.Repeat("x", int(decompressedSize))
	...
	if !errors.Is(readErr, net.ErrDecompressedTooLarge) {
...
func TestDecompressionBombSkippedWhenLimitDisabled(t *testing.T) {
	...
	decompressedSize := int64(10*net.MaxDecompressionRatio + 1)
	body := strings.Repeat("x", int(decompressedSize))
...
func TestLimitedContextReaderDecompressionBombExactBoundary(t *testing.T) {
	...
	compressedSize := int64(100)
	ratio := net.MaxDecompressionRatio
	decompressedSize := compressedSize * ratio

	reader := net.NewLimitedContextReaderWithCompressedSize(
		context.Background(),
		strings.NewReader(strings.Repeat("x", int(decompressedSize))),
		0,
		compressedSize,
	)
	_, err := io.ReadAll(reader)
...
func TestLimitedContextReaderBodyLimitAndDecompressionBomb(t *testing.T) {
	...
	compressedSize := int64(1000)
	bodyLimit := int64(500)
	decompressedSize := int64(1000*net.MaxDecompressionRatio + 1) // bomb
...
func TestFetchStreamContentLengthPreCheckWithDefaultLimit(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		resp := newTestResponse(req, http.StatusOK, "won't read")
		resp.ContentLength = int64(net.DefaultMaxBodySize + 1)
		return resp, nil
	})}
	service := net.NewService(net.ServiceOptions{Client: client})
...
func BenchmarkFetchWithMetaRedirectLimit(b *testing.B) {
	server := redirectServer(net.MaxRedirects+5, "should not reach")
	defer server.Close()
	service := net.NewService(net.ServiceOptions{})
...
func TestServiceMIMEValidationFetchWithMeta(t *testing.T) {
	...
	if !errors.Is(err, net.ErrUnsupportedMediaType) {
...
func TestServiceFetcherCSP(t *testing.T) {
	...
	fetcher := net.NewFetcherWithService(net.NewService(net.ServiceOptions{}))
