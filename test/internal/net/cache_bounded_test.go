...
func newBoundedCache(t *testing.T, maxEntries int, maxBytes int64) *net.HTTPCache {
	t.Helper()
	cfg := net.HTTPCacheConfig{
		Root:       t.TempDir(),
		Private:    false,
		MaxEntries: maxEntries,
		MaxBytes:   maxBytes,
	}
	return net.NewHTTPCacheWithConfig(cfg)
}

func putEntry(t *testing.T, c *net.HTTPCache, url string, body string, maxAge int) {
	t.Helper()
...
func assertMiss(t *testing.T, c *net.HTTPCache, url string) {
	t.Helper()
	_, _, ok := c.Get(url)
	if ok {
...
func TestHTTPCacheBoundedPrivateModeStillBlocked(t *testing.T) {
	...
	c := net.NewHTTPCacheWithConfig(cfg)
...
func TestHTTPCacheBoundedNewHTTPCacheBackwardCompatibility(t *testing.T) {
	c := net.NewHTTPCache(t.TempDir(), false)
...
func BenchmarkHTTPCachePut(b *testing.B) {
	dir := b.TempDir()
	c := net.NewHTTPCacheWithConfig(net.HTTPCacheConfig{
		Root:       dir,
		MaxEntries: 256,
	})
