...
func TestValidateContentType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := net.ValidateContentType(tt.raw, tt.allowed)
			if tt.wantErr {
				if err == nil {
...
func TestParseMediaType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.raw, func(t *testing.T) {
			got := net.ParseMediaType(tt.raw)
			if got != tt.want {
				t.Errorf("net.ParseMediaType(%q) = %q, want %q", tt.raw, got, tt.want)
			}
		})
	}
}
...
func TestClassifyContentType(t *testing.T) {
	tests := []struct {
		raw  string
		want net.MediaTypeClass
	}{
		{"text/html", net.MediaTypeHTML},
		{"text/html; charset=utf-8", net.MediaTypeHTML},
		{"Text/HTML", net.MediaTypeHTML},
		{"text/css", net.MediaTypeCSS},
		{"text/css; charset=utf-8", net.MediaTypeCSS},
		{"text/javascript", net.MediaTypeJavaScript},
		{"application/javascript", net.MediaTypeJavaScript},
		{"application/x-javascript", net.MediaTypeJavaScript},
		{"text/x-javascript", net.MediaTypeJavaScript},
		{"image/png", net.MediaTypeImage},
		{"image/jpeg", net.MediaTypeImage},
		{"image/gif", net.MediaTypeImage},
		{"image/webp", net.MediaTypeImage},
		{"image/avif", net.MediaTypeImage},
		{"image/bmp", net.MediaTypeImage},
		{"image/x-icon", net.MediaTypeImage},
		{"image/svg+xml", net.MediaTypeImage},
		{"application/json", net.MediaTypeUnknown},
		{"application/octet-stream", net.MediaTypeUnknown},
		{"", net.MediaTypeUnknown},
		{"   ", net.MediaTypeUnknown},
		{"video/mp4", net.MediaTypeUnknown},
	}

	for _, tt := range tests {
		t.Run(tt.raw, func(t *testing.T) {
			got := net.ClassifyContentType(tt.raw)
			if got != tt.want {
...
func TestMediaTypeClassString(t *testing.T) {
	tests := []struct {
		class net.MediaTypeClass
		want  string
	}{
		{net.MediaTypeHTML, "html"},
		{net.MediaTypeCSS, "css"},
		{net.MediaTypeJavaScript, "javascript"},
		{net.MediaTypeImage, "image"},
		{net.MediaTypeUnknown, "unknown"},
		{net.MediaTypeClass(99), "unknown"},
	}
	for _, tt := range tests {
...
func BenchmarkValidateContentType(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		_ = net.ValidateContentType(raw, allowed)
	}
}
