// Package memory_test — M9 performance target: cache eviction is observable and
...
func TestEvictionObservable_ComponentLimit(t *testing.T) {
	...
	m.UpdateUsage(mem.ComponentImage, limit+over)
	...
	if got := m.Usage(mem.ComponentImage); got != limit {
...
func TestEvictionObservable_GlobalOrder(t *testing.T) {
	...
	m := mem.NewManager(mem.Config{GlobalLimit: globalLimit})
	m.SetEvictionOrder([]mem.Component{
		mem.ComponentNetworkCache,
		mem.ComponentStyle,
		mem.ComponentImage,
	})
	...
	m.UpdateUsage(mem.ComponentStyle, 900)
	m.UpdateUsage(mem.ComponentImage, 800)
...
func TestEvictionObservable_StatsSnapshotConsistent(t *testing.T) {
	...
	m := mem.NewManager(mem.Config{
		Limits: map[mem.Component]uint64{
			mem.ComponentImage: imgLimit,
			mem.ComponentDOM:   2000,
		},
	...
	m.RegisterEvictor(mem.ComponentDOM, func(target uint64) uint64 {
		return target
	})

	m.UpdateUsage(mem.ComponentImage, 700) // 200 B over limit — triggers component eviction
	m.UpdateUsage(mem.ComponentDOM, 1000)

	stats := m.Stats()

	// Limits snapshot must match configuration.
	if got := stats.Limits[mem.ComponentImage]; got != imgLimit {
		t.Errorf("Stats.Limits[Image] = %d, want %d", got, imgLimit)
	}
	if got := stats.Limits[mem.ComponentDOM]; got != 2000 {
	...
	if got := stats.Usage[mem.ComponentImage]; got > imgLimit {
...
func TestEvictionObservable_ZeroLimitNoEviction(t *testing.T) {
	...
	m := mem.NewManager(mem.Config{}) // no limits set
	m.RegisterEvictor(mem.ComponentImage, func(target uint64) uint64 {
		evictorCalled.Store(true)
		return target
	})

	m.UpdateUsage(mem.ComponentImage, 1<<30) // 1 GB — way over any reasonable limit
...
func BenchmarkEvictionObservable_ComponentLimit(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if i%2 == 0 {
			m.UpdateUsage(mem.ComponentImage, 1500) // over limit → eviction
		} else {
			m.UpdateUsage(mem.ComponentImage, 500) // under limit → no eviction
		}
	}
}
...
func BenchmarkEvictionObservable_GlobalCascade(b *testing.B) {
	m := mem.NewManager(mem.Config{GlobalLimit: 1000})
	m.SetEvictionOrder([]mem.Component{mem.ComponentNetworkCache, mem.ComponentStyle})
	m.RegisterEvictor(mem.ComponentNetworkCache, func(target uint64) uint64 {
	...
	for i := 0; i < b.N; i++ {
		m.UpdateUsage(mem.ComponentNetworkCache, 800)
		m.UpdateUsage(mem.ComponentStyle, 600) // total=1400, 400 over global limit
		// Reset for next iteration
		m.UpdateUsage(mem.ComponentNetworkCache, 0)
		m.UpdateUsage(mem.ComponentStyle, 0)
	}
}
