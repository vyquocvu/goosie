...
func TestScreenshot_BasicRender(t *testing.T) {
	...
	info, err := svc.CreateContext(ctx, browsercontrol.CreateContextOptions{
		Viewport: browsercontrol.Viewport{Width: 320, Height: 240},
	})
	require.NoError(t, err)
	ec, err := svc.Context(context.Background(), info.ID)
	require.NoError(t, err)

	result, err := ec.Screenshot(ctx, browsercontrol.ScreenshotOptions{})
	require.NoError(t, err)
	...
	assert.LessOrEqual(t, len(result.Data), browsercontrol.MaxScreenshotEncoded)
}
...
func TestScreenshot_HugeViewport(t *testing.T) {
	...
	info, err := svc.CreateContext(ctx, browsercontrol.CreateContextOptions{
		Viewport: browsercontrol.Viewport{Width: 10000, Height: 10000}, // 100MP
	})
	require.NoError(t, err)
	ec, err := svc.Context(context.Background(), info.ID)
	require.NoError(t, err)
	...
	assert.Less(t, result.Width*result.Height, browsercontrol.MaxScreenshotPixels+1)
}
...
func TestScreenshot_AfterNavigate(t *testing.T) {
	...
	info, err := svc.CreateContext(ctx, browsercontrol.CreateContextOptions{
		Viewport: browsercontrol.Viewport{Width: 800, Height: 600},
	})
	require.NoError(t, err)
	ec, err := svc.Context(context.Background(), info.ID)
	require.NoError(t, err)
...
func TestClick_NotFoundWithoutDoc(t *testing.T) {
	...
	ref := browsercontrol.ElementRef{Ref: "no-such-ref", ContextID: info.ID, PageRevision: 0}
	_, err = ec.Click(ctx, ref, browsercontrol.ClickOptions{})
	assert.Error(t, err)
}
...
func TestStubClick_WrongContext(t *testing.T) {
	...
	infoA, _ := svc.CreateContext(ctx, browsercontrol.CreateContextOptions{})
	infoB, _ := svc.CreateContext(ctx, browsercontrol.CreateContextOptions{})

	ecA, _ := svc.Context(context.Background(), infoA.ID)
	ref := browsercontrol.ElementRef{Ref: "e1", ContextID: infoB.ID, PageRevision: 0}

	_, err := ecA.Click(ctx, ref, browsercontrol.ClickOptions{})
	require.Error(t, err)
...
func TestStubClick_RevisionMismatch(t *testing.T) {
	...
	ref := browsercontrol.ElementRef{Ref: "e1", ContextID: info.ID, PageRevision: 999}
	_, err := ec.Click(ctx, ref, browsercontrol.ClickOptions{})
	require.Error(t, err)
...
func TestClick_Cancelled(t *testing.T) {
	...
	defer freshCancel()
	info, err := svc.CreateContext(fresh, browsercontrol.CreateContextOptions{})
	require.NoError(t, err)
	ec, err := svc.Context(context.Background(), info.ID)
	require.NoError(t, err)
	...
	callCancel()
	ref := browsercontrol.ElementRef{Ref: "e1", ContextID: info.ID, PageRevision: 0}
	_, err = ec.Click(callCtx, ref, browsercontrol.ClickOptions{})
	require.Error(t, err)
}
...
func TestType_WrongContext(t *testing.T) {
	...
	ecA, _ := svc.Context(context.Background(), infoA.ID)
	ref := browsercontrol.ElementRef{Ref: "i1", ContextID: infoB.ID, PageRevision: 0}
	_, err := ecA.Type(ctx, ref, "hello", browsercontrol.TypeOptions{})
	require.Error(t, err)
}
...
func TestStubType_RevisionMismatch(t *testing.T) {
	...
	ref := browsercontrol.ElementRef{Ref: "i1", ContextID: info.ID, PageRevision: 999}
	_, err := ec.Type(ctx, ref, "hello", browsercontrol.TypeOptions{})
	require.Error(t, err)
}
...
func TestType_Cancelled(t *testing.T) {
	...
	callCtx, callCancel := context.WithCancel(context.Background())
	callCancel()
	ref := browsercontrol.ElementRef{Ref: "i1", ContextID: info.ID, PageRevision: 0}
	_, err = ec.Type(callCtx, ref, "x", browsercontrol.TypeOptions{})
	require.Error(t, err)
}
