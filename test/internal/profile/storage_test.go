...
func TestStorageStoreIsOriginScopedAndPersistent(t *testing.T) {
	p, err := profile.Open(profile.Options{Root: t.TempDir()})
	require.NoError(t, err)
	defer p.Close()

	store, err := profile.NewStorageStore(p)
	require.NoError(t, err)
	require.NoError(t, store.Set("https://one.test", "theme", "dark"))
	require.NoError(t, store.Set("https://two.test", "theme", "light"))

	reloaded, err := profile.NewStorageStore(p)
	require.NoError(t, err)
...
func TestStorageStoreConcurrentInstancesMergeSets(t *testing.T) {
	...
	first, err := profile.NewStorageStore(p)
	require.NoError(t, err)
	second, err := profile.NewStorageStore(p)
	require.NoError(t, err)
...
func TestStorageStoreConcurrentIndependentInstancesMergeSets(t *testing.T) {
	...
	const count = 20
	stores := make([]*profile.StorageStore, count)
	for i := range stores {
		stores[i], err = profile.NewStorageStore(p)
		require.NoError(t, err)
	}
	...
	for i, store := range stores {
		wg.Add(1)
		go func(i int, store *profile.StorageStore) {
			defer wg.Done()
			<-start
...
func TestStorageStorePrivateDoesNotPersist(t *testing.T) {
	root := t.TempDir()
	p, err := profile.Open(profile.Options{Root: root, Private: true})
	require.NoError(t, err)
	defer p.Close()
	...
	normal, err := profile.Open(profile.Options{Root: root})
	require.NoError(t, err)
	defer normal.Close()
	reloaded, err := profile.NewStorageStore(normal)
	require.NoError(t, err)
