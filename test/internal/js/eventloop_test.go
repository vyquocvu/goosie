...
func TestNewEventLoop(t *testing.T) {
	el := js.NewEventLoop(js.DefaultEventLoopConfig())
...
func TestEventLoop_TimerBounded(t *testing.T) {
	cfg := js.EventLoopConfig{MaxTimers: 2, MaxTasks: 10, MaxMicrotasks: 10}
	el := js.NewEventLoop(cfg)
...
func TestEventLoop_TypedMutationBatch(t *testing.T) {
	...
	var got []js.DOMMutation
	el.SetMutationBatchFlush(func(batch []js.DOMMutation) {
		got = batch
	})
	el.QueueTask(func() {
		el.RecordMutation()
	...
	if got[0].Kind != js.MutationBatch || got[0].Count != 2 {
		t.Fatalf("batch = %#v, want kind=%v count=2", got[0], js.MutationBatch)
	}
}
...
func TestEventLoop_TaskQueueFull(t *testing.T) {
	cfg := js.EventLoopConfig{MaxTasks: 2, MaxMicrotasks: 10, MaxTimers: 10}
	el := js.NewEventLoop(cfg)
...
func TestEventLoop_MicrotaskQueueFull(t *testing.T) {
	cfg := js.EventLoopConfig{MaxTasks: 10, MaxMicrotasks: 2, MaxTimers: 10}
	el := js.NewEventLoop(cfg)
