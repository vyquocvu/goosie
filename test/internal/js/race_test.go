...
func TestRace_RepeatedNavigationDuringTimers(t *testing.T) {
	session := js.NewSession(js.DefaultSessionConfig())
	go func() {
		_ = session.Run()
	}()
	defer session.Close()
	...
	// Start timers
	err := session.Submit(func(rt *js.Runtime) {
		_, _ = rt.RunScript(`
			for (var i = 0; i < 20; i++) {
	...
	for i := 0; i < 5; i++ {
		...
		if err != nil && !errors.Is(err, js.ErrSessionClosed) {
...
func TestRace_TabCloseWithPendingTasks(t *testing.T) {
	...
	session := js.NewSessionWithContext(ctx, js.DefaultSessionConfig())
	...
	for i := 0; i < 50; i++ {
		_ = session.Submit(func(rt *js.Runtime) {
